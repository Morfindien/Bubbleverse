#!/usr/bin/env python3
"""Bubbleverse Q041 V15 — segmented exact-resume with unambiguous final-outcome semantics.

Scientific model/data/priors/sampler thresholds/classification logic are inherited
unchanged from Q041-PLANCKPORT-V14. V15 changes only final-outcome semantics:
controlled preregistered numerical non-completion becomes a validated green FINAL
NO_SCIENTIFIC_RESULT, while technical failures remain hard failures (red).
"""
...
Q="Q-041"
PROGRAM_ID="Q041-PLANCKPORT-V15"
RUN_ID="Q041-DOWNSTREAM-SCIENTIFIC-CONSEQUENCE-PORTABILITY-V15"
RESULT_ID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-015"
V14_PROGRAM_ID="Q041-PLANCKPORT-V14"
V13_PROGRAM_ID="Q041-PLANCKPORT-V13"
V13_PARENT_RUN_ID=34379785115
CHAIN_META="q041_chain_metadata_v15.json"
V14_CHAIN_META="q041_chain_metadata_v14.json"
...
def _adopt_v14_metadata(output_dir: str|Path, implementation: str, combo: str, chain: int) -> None:
    """Adopt V14 metadata/state identity without changing the chain/checkpoint bytes."""
    outdir=Path(output_dir); dst=outdir/CHAIN_META; src=outdir/V14_CHAIN_META
    if dst.exists() or not src.exists():
        return
    try: d=base.read_json(src)
    except Exception: return
    if not (d.get("q")==Q and d.get("program_id")==V14_PROGRAM_ID
            and d.get("implementation")==implementation and d.get("combo")==combo
            and int(d.get("chain",-1))==int(chain)
            and d.get("status") in ("COMPLETE","PARTIAL_SOFT_STOP")):
        return
    prior=d.get("status")
    d["program_id"]=PROGRAM_ID; d["run_id"]=RUN_ID; d["result_id"]=RESULT_ID
    d["technical_parent_program_id"]=V14_PROGRAM_ID
    d["adopted_from_v14_cache"]=True; d["adopted_v14_status"]=prior
    base.write_json(dst,d)
    print(f"Q041_V14_STATE_ADOPTION_GATE=PASS arm={implementation} combo={combo} chain={chain} status={prior}")
...
def sample(args: argparse.Namespace) -> int:
    ...
    _adopt_v14_metadata(outdir,args.implementation,args.combo,args.chain)
