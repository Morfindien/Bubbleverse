...
Q="Q-041"; PID="Q041-PLANCKPORT-V18"; RID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-018"
...
def static(a):
    ...
    assert tr["scientific_and_numerical_settings_changed_from_v17"] is False
    assert tr["single_workflow_run"] is True and tr["fixed_linear_segment_chain"] is True and tr["self_dispatch_removed"] is True
    assert tr["fresh_v18_segment1_required"] is True and tr["immediate_predecessor_artifact_only"] is True
    ...
    if ent is None: raise AssertionError("REGISTRY_V18_GATE=FAIL missing Q041-PLANCKPORT-V18")
    assert ent["status"]=="ACTIVE" and ent["workflow_id"]=="q041-planck-portability-v18.yml"
    parent=reg.get("programs",{}).get("Q041-PLANCKPORT-V17")
    if parent is None: raise AssertionError("REGISTRY_V17_PARENT_GATE=FAIL missing Q041-PLANCKPORT-V17")
    ...
    for n in range(1,9):
        ...
        if n>1:
            ...
            assert f"-seg{n-1}-v18" in wf
        assert f"--segment '{n}'" in wf and f"-seg{n}-v18" in wf
    ...
    for token in ["max-parallel: 6","matrix:","implementation: [camspec, hillipop]","chain: [0, 1]",
                  "FINAL — Q041 VALID OUTCOME","finalize-linear","q041-final-v18","q041-environment-v18",
                  "q032-preflight-sealed-v2","q032-hillipop-covariance-v2","actions/download-artifact@v4"]:
        assert token in wf, token
    for forbidden in ["/dispatches","Dispatch next","inputs.segment","inputs.series_id","q041-state-v18-","V16_PARENT_SERIES_ID"]:
        assert forbidden not in wf, forbidden
    for token in ["V18_FRESH_SEGMENT1_GATE","V18_LINEAR_PREDECESSOR_PROVENANCE_GATE","_lock_hillipop_resume_info",
                  "V18_HILLIPOP_RESUME_LOCK_GATE","cobaya_run(info,resume=True,force=False)","cobaya_run(info,resume=False,force=True)",
    ...
    dump(a.output,{"q":Q,"program_id":PID,"stage":"STATIC_TESTS","status":"PASS","gates":{
      ...
      "NO_SELF_DISPATCH":"PASS","IMMEDIATE_PREDECESSOR_ARTIFACT":"PASS","HILLIPOP_EXACT_RESUME_LOCK":"PASS",
      "NO_ALLOW_CHANGES":"PASS","MATRIX_32_X_8":"PASS","TECHNICAL_FAILURE_RED":"PASS","CONTROLLED_NO_SCIENCE_GREEN_FINAL":"PASS"}})
    print("Q041_V18_STATIC_TEST_GATE=PASS"); return 0
def final(a):
    ...
    print(f"Q041_V18_FINAL_TEST_GATE=PASS classification={cls}"); return 0
def main():
