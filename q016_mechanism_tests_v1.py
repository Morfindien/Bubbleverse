...
Q="Q016"; RUN="Q016-CMB-MECHANISM-V1"; RESULT="R-Q016-EDE-CMB-MECHANISM-001"
...
def static(cfgp,outp):
    ...
    src=Path("q016_mechanism_v1.py").read_text()
    agg=Path("q016_mechanism_aggregate_v1.py").read_text()
    wf=Path(".github/workflows/q016-cmb-mechanism-v1.yml").read_text() if Path(".github/workflows/q016-cmb-mechanism-v1.yml").exists() else Path("q016-cmb-mechanism-v1.yml").read_text()
    ...
    checks={
        ...
        "V16_SETUP_BRANCH_ARGUMENT_GATE":
            'bash q016_mechanism_setup_v1.sh "${{ matrix.branch }}"' in wf,
        "COBAYA_PACKAGES_PATH_GATE":
            'COBAYA_PACKAGES_PATH: ${{ github.workspace }}/external/cobaya_packages' in wf,
        "ACT_SHARED_ASSET_SEED_GATE":
            'q016_act_data_v1/data/ACTDR6CMBonly/v1.0/dr6_data_cmbonly.fits' in Path("q016_mechanism_setup_v1.sh").read_text(),
        "FROZEN_STACK_POST_SETUP_GATE":
            "Q016_MECHANISM_POST_SETUP_FROZEN_STACK_GATE=PASS" in Path("q016_mechanism_setup_v1.sh").read_text(),
    }
