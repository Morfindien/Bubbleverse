#!/usr/bin/env python3
"""Static and scientific-result gates for Bubbleverse Q014 V5."""
...
def static(config:str,source_lock:str)->dict[str,Any]:
    ...
    try:
        nuis=c["chains"]["spt_d1_only"]["primary_likelihood"]["nuisance_parameters"]
        gates["SPT_NATIVE_CALIBRATION_NUISANCE_GATE"]=(
            set(nuis)=={"Tcal","Ecal"} and
            float(nuis["Tcal"]["prior"]["min"])==0.8 and float(nuis["Tcal"]["prior"]["max"])==1.2 and
            float(nuis["Ecal"]["prior"]["min"])==0.8 and float(nuis["Ecal"]["prior"]["max"])==1.2 and
            float(nuis["Tcal"]["gaussian_constraint"]["mean"])==1.0 and
            float(nuis["Tcal"]["gaussian_constraint"]["sigma"])==0.0036 and
            nuis["Ecal"].get("gaussian_constraint") is None)
    except Exception:
        gates["SPT_NATIVE_CALIBRATION_NUISANCE_GATE"]=False
    main=(ROOT/"q014_external_viability_v5.py").read_text(encoding="utf-8")
    gates["SPT_CALIBRATION_PROFILE_DESIGN_GATE"]=(
        'params[name] = {' in main and
        'priors["q014_spt_gaussian_Tcal"]' in main and
        '{"Tcal", "Ecal"}.issubset(set(sampled))' in main)
    ...
    gates["SPT_OFFICIAL_LITE_CONFIG_SOURCE_GATE"]=(
        s["spt_k040"].get("official_sptlite_cobaya_config",{}).get("git_blob")=="f3b3e15ea512931e28e130ef62ef35bb96171719" and
        s["spt_k040"].get("official_sptlite_cobaya_config",{}).get("pinned_commit")=="2cec6e762a8c540484dd5acafc529f0035856350")
    gates["CANDL_DISTRIBUTION_IDENTITY_GATE"]=(
        s["spt_k040"].get("candl_distribution")=="candl-like" and
        s["spt_k040"].get("candl_import_module")=="candl" and
        s["spt_k040"].get("candl_version")=="2.0.3" and
        c["chains"]["spt_d1_only"]["primary_likelihood"].get("distribution")=="candl-like" and
        c["chains"]["spt_d1_only"]["primary_likelihood"].get("import_module")=="candl")
    setup=(ROOT/"q014_setup_v5.sh").read_text(encoding="utf-8")
    gates["CANDL_SETUP_INSTALL_GATE"]=(
        'pip install "candl-like==2.0.3"' in setup and
        "md.version('candl-like')" in setup and
        'pip install "candl==2.0.3"' not in setup)
    gates["POST_EXTERNAL_INSTALL_Q005_PIN_GATE"]="Q014_POST_EXTERNAL_INSTALL_Q005_PIN_GATE=PASS" in setup
    gates["CLASS_EDE_PERSISTENT_IMPORT_DESIGN_GATE"]=(
        "bubbleverse_q014_class_ede.pth" in setup and
        "Q014_CLASS_EDE_PERSISTENT_IMPORT_GATE=PASS" in setup and
        "unset PYTHONPATH" in setup and
        "site.getsitepackages()" in setup)
...
def result_tests(config:str,result_path:str)->dict[str,Any]:
    ...
    gates["Q_IDENTITY_GATE"]=r.get("q")=="Q014" and r.get("run_id")=="Q014-EXTERNAL-VIABILITY-V5"
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("--config",default="q014_external_viability_v5_config.yml")
    sub=ap.add_subparsers(dest="cmd",required=True)
    p=sub.add_parser("static"); p.add_argument("--source-lock",default="q014_external_viability_v5_source_lock.json"); p.add_argument("--output",required=True)
