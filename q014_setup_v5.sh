...
mkdir -p external q014_v5_source_runtime "$COBAYA_PACKAGES_PATH"
...
# V5: GitHub Actions executes later workflow steps in fresh shells. A shell-local
# PYTHONPATH therefore cannot be the transport mechanism for the frozen CLASS_EDE
# Python extension. Register the exact build directory in this job's active Python
# environment using a .pth file, which Python automatically loads in every later step.
SITE_PACKAGES="$(python - <<'PY'
import site
paths=site.getsitepackages()
if not paths: raise SystemExit('No site-packages directory found')
print(paths[0])
PY
)"
CLASS_PTH="$SITE_PACKAGES/bubbleverse_q014_class_ede.pth"
printf '%s\n%s\n%s\n' "$BUILD_LIB" "$CLASS_DIR" "$CLASS_DIR/python" > "$CLASS_PTH"

# Prove persistence without relying on this setup shell's PYTHONPATH.
unset PYTHONPATH || true
CLASS_DIR_EXPECTED="$CLASS_DIR" CLASS_PTH_EXPECTED="$CLASS_PTH" python - <<'PY'
import os, pathlib, classy, numpy, Cython
assert numpy.__version__=='1.26.4'; assert Cython.__version__=='0.29.37'
actual=pathlib.Path(classy.__file__).resolve()
expected=pathlib.Path(os.environ['CLASS_DIR_EXPECTED']).resolve()
assert str(actual).startswith(str(expected) + os.sep), (actual, expected)
pth=pathlib.Path(os.environ['CLASS_PTH_EXPECTED'])
assert pth.is_file() and str(actual.parent) in pth.read_text(), (actual, pth)
print('Q014_CLASS_EDE_PERSISTENT_IMPORT_GATE=PASS',actual)
PY
...
PY
  ...
  printf '%s  %s\n' "$MUSE_SHA" "$MUSE_ZIP" | tee q014_v5_source_runtime/muse_3g_like_march_2025.sha256
...
import classy
out['classy_module']=getattr(classy,'__file__',None)
out['classy_persistence']='site-packages .pth -> frozen external/class_ede build/lib.*'
if chain.startswith('spt_') or chain=='all':
    ...
    out['muse_sha256']=pathlib.Path('q014_v5_source_runtime/muse_3g_like_march_2025.sha256').read_text().split()[0]
pathlib.Path('q014_v5_source_runtime/setup_imports.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
