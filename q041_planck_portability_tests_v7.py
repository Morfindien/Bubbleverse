...
Q='Q-041'; PID='Q041-PLANCKPORT-V7'; RID='R-Q041-EDE-DOWNSTREAM-PORTABILITY-007'
...
def static(a):
    ...
    assert ent['q']==Q and ent['status']=='ACTIVE' and ent['workflow_id']=='q041-planck-portability-v7.yml'
    assert ent['scientific_program']=='q041_planck_portability_v7.py'
    wf=pathlib.Path(a.workflow).read_text(encoding='utf-8')
    for token in [PID,'q041_prepare_frozen_core_v7.sh','prepare-nonoverlap','runtime-preflight','P_A6_L6_D2','P_L6_D2','P_A6_D2','P_A6_L6','aggregate']:
    ...
    setup=pathlib.Path('q041_setup_v7.sh').read_text(encoding='utf-8')
    core=pathlib.Path('q041_prepare_frozen_core_v7.sh').read_text(encoding='utf-8')
    ...
    assert 'name: q041-chain-${{ matrix.implementation }}-${{ matrix.combo }}-c${{ matrix.chain }}-v7' in wf
    assert 'pattern: q041-chain-*-v7' in wf
