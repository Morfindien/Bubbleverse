#!/usr/bin/env python3
"""Bubbleverse Q041 — downstream scientific-consequence portability V1.
...
Q="Q-041"
PROGRAM_ID="Q041-PLANCKPORT-V1"
RUN_ID="Q041-DOWNSTREAM-SCIENTIFIC-CONSEQUENCE-PORTABILITY-V1"
RESULT_ID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-001"
...
def q032_cfg(q32: Any, root: str|Path) -> dict[str,Any]:
    c=q32.load_cfg(Path(root)/"q032_planck_tt3pair_bridge_v2_config.yml")
...
def sample(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_json(Path(args.output_dir)/"q041_chain_metadata_v1.json",rec)
...
def aggregate(args: argparse.Namespace) -> int:
    load_preregister(args.preregister); load_source_lock(args.source_lock)
    metas=[]
    for p in Path(args.input_dir).rglob("q041_chain_metadata_v1.json"):
        d=read_json(p)
