#!/usr/bin/env python3
"""Aggregate Q014 V9 basin-replication jobs using the cross-mode-comparable profile deviance."""
from __future__ import annotations
import argparse,json,math
from pathlib import Path
import yaml
from q014_external_viability_v9 import impl
ROOT=Path(__file__).resolve().parent
CONFIG_NAME="q014_external_viability_v9_config.yml"
RUN="Q014-EXTERNAL-VIABILITY-V9"
...
def aggregate(records,c,depth):
    ...
    result={"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN,"result_id":c["project"]["result_id"],"original_case_question":c["project"]["original_case_question"],
      "program_status":"V9 V8-BASIN REPLICATION RESULTS AGGREGATED","depth":depth,"q011_globality_status_preserved":c["claim_boundary"]["q011_globality_status_preserved"],
    ...
    result["journal_update"]={"case_id":c["project"]["case_id"],"q":"Q014","program":"q014_external_viability_v9.py","workflow":"q014-external-viability-v9.yml","validation_status":result["validation_status"],"what_was_computed":"V9 cross-mode-comparable external n=3 profile deviance with V8 two-basin exact+jitter replication certification.","scientific_consequence":"Interpret only after mandatory V9 validation passes; report each chain separately.","what_remains_unresolved":[] if result["scientifically_usable_result"] else ["One or more primary V9 basin-replication profiles remain numerically unstable or fail nesting monotonicity."],"next_required_action":"ROUTE_VALIDATED_Q014_RESULT" if result["scientifically_usable_result"] else "CONTINUE_NUMERICAL_CERTIFICATION"}
    return result
