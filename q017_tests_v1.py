...
def static(config, source_lock, workflow, output):
    ...
    src=Path("q017_planck_direction_localization_v1.py").read_text(encoding="utf-8")
    setup=Path("q017_setup_v1.sh").read_text(encoding="utf-8")
    ...
    checks={
      ...
      "SETUP_FULL_MF_ONLY_GATE":"cobaya-install planck_NPIPE_highl_CamSpec.TTTEEE" in setup,
      ...
      "SOURCE_LOCK_HEAD_GATE":sl["repository_head_at_design"]=="91bc0736a00e2454d4e37324e5d3a1a7fe7813d6",
    }
    blob_checks={}
    ...
    out={"q":Q,"run_id":RUN,"result_id":RESULT,"test_id":"Q017-V1-STATIC","status":status,"checks":checks,"source_blob_checks":blob_checks}
    write(output,out)
...
def final(result, output):
    ...
    out={"q":Q,"run_id":RUN,"result_id":RESULT,"test_id":"Q017-V1-FINAL","status":status,"checks":checks,"final_result_gate":gate}
    write(output,out)
    if status!="PASS": raise SystemExit(2)
