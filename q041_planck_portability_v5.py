...
Q="Q-041"
PROGRAM_ID="Q041-PLANCKPORT-V5"
RUN_ID="Q041-DOWNSTREAM-SCIENTIFIC-CONSEQUENCE-PORTABILITY-V5"
RESULT_ID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-005"
...
def sample(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_json(Path(args.output_dir)/"q041_chain_metadata_v5.json",rec)
...
def aggregate(args: argparse.Namespace) -> int:
    load_preregister(args.preregister); load_source_lock(args.source_lock)
    metas=[]
    for p in Path(args.input_dir).rglob("q041_chain_metadata_v5.json"):
        d=read_json(p)
