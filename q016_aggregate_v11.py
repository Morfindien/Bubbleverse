#!/usr/bin/env python3
"""Q016 V11 targeted Planck-free multistart continuation aggregator."""
import argparse,json,math
from pathlib import Path
import yaml

Q="Q016"; RUN="Q016-MATCHED-CMB-SURFACE-V11"; RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-011"
...
def main_final(a,cfg):
    ...
    for b in BRANCHES:
        ...
        if b=="planck":
            ...
            if len(extension)!=4: raise RuntimeError(f"V11_PLANCK_EXTENSION_COUNT_GATE=FAIL {len(extension)}")
            for r in extension:
    ...
    gates.update({
      ...
      "V11_PLANCK_EXTENSION_COMPLETE_GATE":len(extension_records)==4,
      "V11_NONZERO_PERTURBATION_GATE":all((r.get("source_vector_meta") or {}).get("nonzero_perturbation") is True for r in extension_records),
    })
    ...
    result={"case_id":cfg["project"]["case_id"],"q":Q,"original_case_question":cfg["scientific_question"],
            ...
            "interpretation_guard":"Branch-local fixed-minus-free objectives only; no cross-chain sum. V11 changes no scientific threshold."}
    dump(a.output,result)
    if not primary: raise SystemExit(2)

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--config",default="q016_matched_cmb_surface_v11_config.yml")
