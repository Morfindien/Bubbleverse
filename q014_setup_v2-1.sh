...
mkdir -p external q014_v2_source_runtime "$COBAYA_PACKAGES_PATH"
...
if [[ "$CHAIN" == spt_d1_* || "$CHAIN" == "all" ]]; then
  python -m pip install "candl==2.0.3"
  ...
  printf '%s  %s\n' "$MUSE_SHA" "$MUSE_ZIP" | tee q014_v2_source_runtime/muse_3g_like_march_2025.sha256
...
if chain.startswith('spt_') or chain=='all':
    ...
    out['candl_version']=md.version('candl')
    out['spt_candl_data_commit']=subprocess.check_output(['git','-C','external/spt_candl_data','rev-parse','HEAD'],text=True).strip()
    out['muse_sha256']=pathlib.Path('q014_v2_source_runtime/muse_3g_like_march_2025.sha256').read_text().split()[0]
pathlib.Path('q014_v2_source_runtime/setup_imports.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
