...
Q="Q016"
RUN="Q016-CMB-MECHANISM-V1"
RESULT="R-Q016-EDE-CMB-MECHANISM-001"
...
def build_certified_point_model(cfg,lock,branch,mode):
    ...
    info=q16.base_info(v16cfg,branch,mode,0,str(ROOT/"_q016_mechanism_eval"))
...
def _residual_packet(delta,labels,cov,bands):
...
def matched_planck_or_spt(cfg,lock,branch):
    ...
    if branch=="planck":
        adapter["likelihood_names"]["planck_highl"]="q016_camspec_npipe_lite"
        func=q15.planck_highl_state
    else:
        adapter["likelihood_names"]["spt_primary"]="q016_spt_d1_lite"
        func=q15.spt_primary_state

    states={}
    for mode in ("reference_free_h0","fixed_h0_71p5"):
        model,vals,lp=build_certified_point_model(cfg,lock,branch,mode)
        state,cap=_trace_return_locals(func,model,vals,adapter)
        delta=cap.get("delta")
        labels=cap.get("labels")
        if delta is None or labels is None:
            raise RuntimeError("Q015_RESIDUAL_CAPTURE_GATE=FAIL")
        d=np.asarray(delta,dtype=float)
...
def matched_act(cfg,lock):
    ...
    for mode in ("reference_free_h0","fixed_h0_71p5"):
        ...
        labels=[]
        # Q009 rows carry exact data-vector indices and spectrum metadata.
        row_by_idx={}
        for r in rows:
            idxs=r.get("idx",r.get("indices",[]))
            if isinstance(idxs,(int,np.integer)): idxs=[int(idxs)]
            for ii in np.asarray(idxs,dtype=int).ravel().tolist():
                row_by_idx[int(ii)]=r
        for i in range(len(residual)):
            r=dict(row_by_idx.get(i,{}))
            r.setdefault("ell",r.get("ell_eff",float("nan")))
            r.setdefault("pol",r.get("type",r.get("spectrum_type","UNKNOWN")))
            r.setdefault("spectrum",r.get("name",r.get("spec","ACT")))
            labels.append(r)
        packet=_residual_packet(residual,labels,cov,bands)
        packet["endpoint_objective_chi2"]=float(endpoint(lock,branch,mode)["objective_chi2"])
        packet["endpoint_primary_chi2"]=_primary_component(endpoint(lock,branch,mode),branch)
        packet["endpoint_h0"]=float(vals["H0"])
...
def _theory_dls_from_model(model):
    ...
    if "ell" not in out:
        n=len(next(v for k,v in out.items() if k!="ell"))
        out["ell"]=np.arange(n,dtype=int)
    return out
...
def secondary_planck(cfg,lock):
    ...
    matched=matched_planck_or_spt(cfg,lock,"planck")
...
def run_matched(cfg,lock,branch):
    if branch=="planck" or branch=="spt": return matched_planck_or_spt(cfg,lock,branch)
