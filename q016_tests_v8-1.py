#!/usr/bin/env python3
"""Q016 V8 static/runtime objective tests and final gate validation."""
import argparse,ast,json,math
from pathlib import Path
import yaml

RUN="Q016-MATCHED-CMB-SURFACE-V8"
RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-008"
...
def static_test(cfg_path,output):
    ...
    src=Path("q016_objective_reprofile_v8.py").read_text()
    ast.parse(src)
    checks={
        ...
        "NO_HARDCODED_ENDPOINT_PASS_GATE":'"ENDPOINT_REPROFILE_GATE":True' not in Path("q016_aggregate_v8.py").read_text().replace(" ",""),
        "PROFILE_NESTING_IMPLEMENTED_GATE":"profile_nesting_pass" in Path("q016_aggregate_v8.py").read_text(),
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V8 objective.
    #
    ...
    # the target lay ~7 sigma outside Cobaya's allowed numerical box. V8 instead
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v8_config.yml");p.add_argument("--output",required=True)
