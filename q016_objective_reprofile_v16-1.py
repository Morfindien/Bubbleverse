#!/usr/bin/env python3
"""Bubbleverse Q016 V16 exact-objective fixed/free reprofile.
...
def source_vector(info,branch,mode,family,cfg,phase1_path=None,parent_free_result=None):
    # V6 history is needed only by legacy parent_* start families.
    # The targeted V10 Planck continuation must not depend on a V6 lock identity.
    lock=None
...
def run_task(args):
    ...
    status.update({"status":"RUNNING","seed_candidate":seed,"source_vector_meta":source_meta,
                   "runtime_provenance_file":f"q016_v16_source_runtime/setup_{branch}.json"})
    write_json(out,status)
...
def main():
    ...
    ap.add_argument("--config",default=str(ROOT/"q016_matched_cmb_surface_v16_config.yml"))
