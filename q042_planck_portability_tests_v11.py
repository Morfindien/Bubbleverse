...
Q="Q-042";PID="Q042-PREFLIGHT-V11"
...
def static(a):
 ...
 gates={
  ...
  "WORKFLOW_IDENTITY":"PROGRAM_ID: Q042-PREFLIGHT-V11" in w and "CURRENT_Q: Q-042" in w,
  ...
  "PILOT_CACHE_FAIL_CLOSED":"q042-v11-${{ runner.os }}-py311-cobaya356-q032-4dc873a-pc1222-pybobyqa150-pantheonplus" in w and "fail-on-cache-miss: true" in w,
  ...
  "MERGE_PREFLIGHT_DEPENDENCY_BOOTSTRAP":("merge-preflight:" in w and "Bootstrap merge Python dependencies" in w and "python -m pip install --disable-pip-version-check PyYAML==6.0.2 numpy==1.26.4" in w[w.index("merge-preflight:"):]),
  "WORKFLOW_SELF_REFERENCE":"--workflow .github/workflows/q042-planck-portability-v11.yml" in w,
  "WORKFLOW_REGISTRY_PATCH_SELF_REFERENCE":"--registry-patch q042_program_registry_patch_v11.json" in w and "--registry-patch q042_program_registry_patch_v7.json" not in w and "--registry-patch q042_program_registry_patch_v6.json" not in w,
  ...
  "ACTIVE_V11_WORKFLOW_OUTPUTS":all(x in w for x in ("q042_static_program_v11.json","q042_static_tests_v11.json","q042_primary_nonoverlap_support_v11.json","q042_hillipop_nonoverlap_precision_v11.npy","q042_runtime_preflight_v11.json","q042_preflight_final_v11.json")),
  "PILOT_RESULT_VERSION":"q042_pilot_result_v11.json" in p and all(x not in p for x in ("q042_pilot_result_v4.json","q042_pilot_result_v5.json","q042_pilot_result_v6.json","q042_pilot_result_v7.json")),
  "REGISTRY_ARTIFACT_VERSION":all(str(x).endswith("v11") or "*-v11" in str(x) for x in entry.get("result_artifacts",[])) and len(entry.get("result_artifacts",[]))==4,
  "REGISTRY_ENTRY":entry.get("q")==Q and entry.get("status")=="ACTIVE" and entry.get("workflow_id")=="q042-planck-portability-v11.yml",
 ...
 print("Q042_V11_STATIC_TEST_GATE=PASS")
def final(a):
 ...
 if status!="PASS":raise SystemExit(2)
 print("Q042_V11_FINAL_TEST_GATE=PASS")
def parser():
