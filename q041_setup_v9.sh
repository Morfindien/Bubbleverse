...
if [[ ! -s "$ACT_CMB_DATA_FILE" ]]; then
  ...
  tar -tzf "$archive" | grep -qx 'v1.0/dr6_data_cmbonly.fits'
...
rec={
 'q':'Q-041','program_id':'Q041-PLANCKPORT-V9','stage':'EXTERNAL_RUNTIME_PROVENANCE','status':'PASS',
...
pathlib.Path('q041_runtime/q041_external_runtime_provenance_v9.json').write_text(json.dumps(rec,indent=2,sort_keys=True)+'\n')
