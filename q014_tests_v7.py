#!/usr/bin/env python3
"""Q014 V7 static and mandatory result validation."""
...
def static():
    c=yaml.safe_load((ROOT/'q014_external_viability_v7_config.yml').read_text()); p=(ROOT/'q014_external_viability_v7.py').read_text(); w=((ROOT/'.github/workflows/q014-external-viability-v7.yml') if (ROOT/'.github/workflows/q014-external-viability-v7.yml').exists() else (ROOT/'q014-external-viability-v7.yml')).read_text(); s=(ROOT/'q014_setup_v7.sh').read_text(); cons=c['scientific_surface']['normalization_free_constraints']
    gates={
      ...
      'MUSE_FROZEN_REUSE_GATE':MUSE in w and 'q014-v7-spt-muse-source' in w and 'q014_setup_v6.sh' in s,
      ...
      'GITHUB_ACTION_VERSION_GATE':'actions/checkout@v5' in w and 'actions/setup-python@v5' in w and 'actions/download-artifact@v5' in w and 'actions/upload-artifact@v5' in w and 'actions/checkout@v7' not in w and 'actions/setup-python@v7' not in w,
...
def result(path):
    c=yaml.safe_load((ROOT/'q014_external_viability_v7_config.yml').read_text()); r=json.loads(Path(path).read_text()); g={}
    g['Q_IDENTITY_GATE']=r.get('q')=='Q014' and r.get('run_id')==RUN and r.get('result_id')=='R-Q014-EDE-EXTERNAL-VIABILITY-004'
