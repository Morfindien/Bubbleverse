#!/usr/bin/env python3
"""Q016 V12 static/runtime objective tests and final gate validation."""
import argparse,ast,json,math
from pathlib import Path
import yaml

RUN="Q016-MATCHED-CMB-SURFACE-V12"
RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-012"
...
def static_test(cfg_path,output):
    ...
    src=Path("q016_objective_reprofile_v12.py").read_text()
    agg=Path("q016_aggregate_v12.py").read_text()
    wf=Path(".github/workflows/q016-matched-cmb-surface-v12.yml").read_text() if Path(".github/workflows/q016-matched-cmb-surface-v12.yml").exists() else Path("q016-matched-cmb-surface-v12.yml").read_text()
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V12 objective.
    #
    ...
    # the target lay ~7 sigma outside Cobaya's allowed numerical box. V12 instead
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v12_config.yml");p.add_argument("--output",required=True)
