...
Q='Q-040'; PROGRAM_ID='Q040-RQMC-V4'; RESULT_ID='R-Q040-EDE-DEFENSIVE-RQMC-CMB-MARGINAL-004'
...
def static(args):
    import q040_defensive_rqmc_v4 as q
    g={}
    ...
    gate(g,'WORKFLOW_PROGRAM_GATE','PROGRAM_ID: Q040-RQMC-V4' in wf and 'default: Q040-RQMC-V4' in wf)
    gate(g,'WORKFLOW_LAUNCH_INPUT_GATE','workflow_dispatch:' in wf and 'program_id:' in wf)
    gate(g,'SEGMENTED_LAUNCHER_COMPATIBILITY_GATE','00-bubbleverse-start-segmented-v5.yml' in wf and "e.get('result_artifacts') == ['q040-rqmc-final-v4']" in wf)
    ...
    gate(g,'WORKFLOW_RESULT_TEST_GATE','q040_defensive_rqmc_tests_v4.py result' in wf)
    gate(g,'WORKFLOW_FAILURE_SEAL_GATE','seal-failure' in wf and 'DEFENSIVE_MARGINALIZATION_NUMERICAL_VALIDATION_FAIL' in src)
    gate(g,'LOCAL_ACTION_DEPENDENCY_REMOVED_GATE','./.github/actions/q040-download-retry' not in wf)
    gate(g,'SAME_RUN_ARTIFACT_RETRY_GATE',wf.count('Same-run artifact transport gate q040dl')==52 and wf.count('Q040_SAME_RUN_ARTIFACT_TRANSPORT_GATE=FAIL dependency=')==52)
    gate(g,'INLINE_RETRY_ATTEMPT_GATE',wf.count('continue-on-error: true')==156 and wf.count('_a1')>=52 and wf.count('_a2')>=52 and wf.count('_a3')>=52)
    gate(g,'ARTIFACT_RETRY_FAIL_CLOSED_GATE',wf.count('if: always()')>=52 and wf.count('Q040_SAME_RUN_ARTIFACT_CONTENT_GATE=FAIL')==52)
    gate(g,'TECHNICAL_FAILURE_NOT_SCIENCE_SEAL_GATE',"needs.integration_gate.result == 'success'" in wf and "needs.coreset_gate.result == 'success'" in wf and "needs.bank_stability.result == 'success'" in wf and "needs.production_gate.result == 'success'" in wf)
    gate(g,'UPSTREAM_EXECUTION_RESULT_GATE','RQMC_UPSTREAM_EXECUTION_GATE=PASS' in wf and 'rqmc_m12' in wf)
    ...
    ts=pr.get('technical_supersession',{})
    gate(g,'V3_TECHNICAL_SUPERSESSION_GATE',ts.get('supersedes_program_id')=='Q040-RQMC-V3' and ts.get('failed_github_run_id')==34256042442 and ts.get('scientific_result_from_failed_run') is False)
    gate(g,'SEED_NAMESPACE_PRESERVATION_GATE',q.RQMC_SEED_NAMESPACE=='Q040-RQMC-V1' and ts.get('sobol_scramble_sequences_changed') is False)
    sparse=np.array([[-np.inf,-12.5],[-3.0,-np.inf]],dtype=float)
    enc=q._encode_sparse_logsum(sparse)
    json.dumps(enc,allow_nan=False)
    dec=q._decode_sparse_logsum(enc)
    gate(g,'SPARSE_LOGSUM_JSON_ROUNDTRIP_GATE',enc==[[None,-12.5],[-3.0,None]] and np.array_equal(np.isneginf(dec),np.isneginf(sparse)) and np.allclose(dec[np.isfinite(dec)],sparse[np.isfinite(sparse)]))
    # Two complementary sparse shard accumulators must merge exactly after JSON null round-trip.
    sa=np.array([[-2.0,-np.inf],[-np.inf,-5.0]],dtype=float)
    sb=np.array([[-np.inf,-3.0],[-4.0,-np.inf]],dtype=float)
    ma=q._combine_logsum(q._decode_sparse_logsum(q._encode_sparse_logsum(sa)),q._decode_sparse_logsum(q._encode_sparse_logsum(sb)))
    expected=np.logaddexp(sa,sb)
    gate(g,'SPARSE_LOGSUM_MERGE_GATE',np.allclose(ma,expected,equal_nan=False) and np.all(np.isfinite(ma)))
    rejected=False
    try: q._encode_sparse_logsum(np.array([[np.inf]]))
    except RuntimeError: rejected=True
    gate(g,'SPARSE_LOGSUM_POSINF_REJECTION_GATE',rejected)
    rejected=False
    try: q._encode_sparse_logsum(np.array([[np.nan]]))
    except RuntimeError: rejected=True
    gate(g,'SPARSE_LOGSUM_NAN_REJECTION_GATE',rejected)
    gate(g,'V2_FAILURE_SOURCE_PROVENANCE_GATE','Q040-RQMC-V2-RUN-34247581888' in sl.get('bubbleverse_technical_evidence',{}))
    gate(g,'V3_FAILURE_SOURCE_PROVENANCE_GATE','Q040-RQMC-V3-RUN-34256042442' in sl.get('bubbleverse_technical_evidence',{}))
    gate(g,'CORESET_VERSION_ALIGNMENT_GATE','q040-rqmc-coreset-${{ matrix.implementation }}-v4' in wf and 'q040_rqmc_${{ matrix.implementation }}_coreset_v4.json' in wf and 'q040_rqmc_${{ matrix.implementation }}_coreset_v2.json' not in wf)
    gate(g,'NO_DUPLICATE_TIMEOUT_LITERAL_GATE','timeout-minutes: 15\n    timeout-minutes: 15' not in wf)
