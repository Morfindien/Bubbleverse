...
cd "$ROOT"
# V18 changes workflow orchestration only; reuse the validated V16 environment installer.
bash "$ROOT/q041_setup_v16.sh"
python - <<'PYSETUP'
import json, pathlib
src=pathlib.Path("q041_runtime/q041_external_runtime_provenance_v16.json")
dst=pathlib.Path("q041_runtime/q041_external_runtime_provenance_v18.json")
d=json.loads(src.read_text(encoding="utf-8"))
assert d.get("q")=="Q-041" and d.get("status")=="PASS" and d.get("cobaya_upgraded") is False
d["program_id"]="Q041-PLANCKPORT-V18"
d["technical_parent_program_id"]="Q041-PLANCKPORT-V17"
d["single_workflow_run"]=True
d["fixed_linear_segment_chain"]=True
d["self_dispatch_removed"]=True
d["fresh_v18_segment1_required"]=True
...
print("Q041_V18_EXTERNAL_RUNTIME_IDENTITY_GATE=PASS")
PYSETUP
