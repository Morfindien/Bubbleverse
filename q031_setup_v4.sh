...
# TECHNICAL VERSION: V4
# Repairs:
# - assert the workflow really supplied Python 3.11;
# - reject cached class_ede extensions built for another Python ABI;
# - write the .pth file only into the writable user-site directory.
...
# The workflow must provide Python 3.11 via actions/setup-python.
STAGE="PYTHON_311"
python - <<'PY311_GATE'
import sys
assert sys.version_info[:2] == (3, 11), sys.version
print("Q031_PYTHON_311_GATE=PASS", sys.version.split()[0], sys.executable)
PY311_GATE
...
# Reuse a cached extension only if it matches the active CPython 3.11 ABI.
PY_ABI="$(python - <<'PYABI'
import sys
...
if [[ -z "$MATCHING_SO" ]]; then
  echo "[INFO] No class_ede extension matching ${PY_ABI}; rebuilding cleanly"
...
test -n "$BUILD_LIB"
MATCHING_SO="$(find "$BUILD_LIB" -maxdepth 1 -type f -name 'classy*.so' -print -quit)"
test -n "$MATCHING_SO"
case "$MATCHING_SO" in
  *cpython-311*) ;;
  *) echo "Q031_CLASS_EDE_ABI_GATE=FAIL so=$MATCHING_SO" >&2; exit 2 ;;
esac

# Never write into /usr/local or the toolcache site-packages. GitHub's runner
# user owns the user-site directory, and new Python processes load this .pth.
USER_SITE="$(python - <<'PYUSERSITE'
import site
print(site.getusersitepackages())
PYUSERSITE
)"
mkdir -p "$USER_SITE"
printf '%s\n%s\n%s\n' "$BUILD_LIB" "$CLASS_DIR" "$CLASS_DIR/python" > "$USER_SITE/bubbleverse_q031_class_ede_v3.pth"

# Make the current shell immediately consistent too.
export PYTHONPATH="$BUILD_LIB:$CLASS_DIR:$CLASS_DIR/python${PYTHONPATH:+:$PYTHONPATH}"

python - <<'PYABICHECK'
import pathlib, sys, classy
p=pathlib.Path(classy.__file__).resolve()
assert sys.version_info[:2] == (3,11), sys.version
assert 'external/class_ede' in str(p).replace('\\','/'), p
assert 'cpython-311' in p.name, p
print('Q031_CLASS_EDE_ABI_GATE=PASS', sys.version.split()[0], p)
PYABICHECK
