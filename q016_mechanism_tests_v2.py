...
Q="Q016"; RUN="Q016-CMB-MECHANISM-V2"; RESULT="R-Q016-EDE-CMB-MECHANISM-002"
...
def static(cfgp,outp):
    ...
    src=Path("q016_mechanism_v2.py").read_text()
    agg=Path("q016_mechanism_aggregate_v2.py").read_text()
    wf=Path(".github/workflows/q016-cmb-mechanism-v2.yml").read_text() if Path(".github/workflows/q016-cmb-mechanism-v2.yml").exists() else Path("q016-cmb-mechanism-v2.yml").read_text()
    ...
    checks={
        ...
        "FROZEN_REQUIREMENTS_BOOTSTRAP_GATE":
            "python -m pip install -r q005_hpc_v14_requirements.txt" in Path("q016_mechanism_setup_v2.sh").read_text(),
        "COBAYA_PRE_V16_GATE":
            "Q016_MECHANISM_V2_PRE_V16_FROZEN_DEPENDENCY_GATE=PASS" in Path("q016_mechanism_setup_v2.sh").read_text(),
        "PREPARE_DEPENDENCY_PREFLIGHT_GATE":
            "Q016_MECHANISM_V2_FROZEN_DEPENDENCY_BOOTSTRAP_PREFLIGHT=PASS" in wf,
        "V1_ACT_ASSET_REUSE_GATE":
            "9908527497" in wf and
            "sha256:df535122e44b2df14a0ba79c2934e9c55c37838b287d2a89070fed5966d2b48f" in wf,
        ...
        "V16_SETUP_BRANCH_ARGUMENT_GATE":
            'bash q016_mechanism_setup_v2.sh "${{ matrix.branch }}"' in wf,
        "COBAYA_PACKAGES_PATH_GATE":
            'COBAYA_PACKAGES_PATH: ${{ github.workspace }}/external/cobaya_packages' in wf,
        "ACT_SHARED_ASSET_SEED_GATE":
            'q016_act_data_v1/data/ACTDR6CMBonly/v1.0/dr6_data_cmbonly.fits' in Path("q016_mechanism_setup_v2.sh").read_text(),
        "FROZEN_STACK_POST_SETUP_GATE":
            "Q016_MECHANISM_V2_POST_SETUP_FROZEN_STACK_GATE=PASS" in Path("q016_mechanism_setup_v2.sh").read_text(),
    }
