...
Q = "Q028"
RUN = "Q028-FULLMF-COMMON-SUPPORT-COUNTERFACTUAL-V3"
RESULT = "R-Q028-EDE-FULLMF-COMMON-SUPPORT-COUNTERFACTUAL-003"
...
def build_common_precision_candidates(
    ...
    for mask in MASKS:
        ...
        for sa, sb in EDGES:
            ek = f"e{sa}{sb}"
            P = edge_precisions[mask][ek]
            keys = edge_keys[mask][ek]
            v2.validate_spd(P, c, f"mask{mask}:{ek}:full")
            Pm, meta, _ = v2.marginal_precision_from_full_precision(
                P, keys, common_keys, c, f"mask{mask}:{ek}"
            )
...
def main() -> int:
    ...
    ap.add_argument("--config", default="q028_common_support_counterfactual_v3_config.yml")
