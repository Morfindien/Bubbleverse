...
# ACTDR6LensLike is a Cobaya InstallableLikelihood with data_path=ACT_dr6_likelihood.
# Therefore the authoritative data must be installed under COBAYA_PACKAGES_PATH,
# not only inside the editable source checkout.
for i in 1 2 3 4; do
  echo "[INFO] Q041 Cobaya install ACT DR6 lensing data attempt=$i/4"
  if cobaya-install act_dr6_lenslike.ACTDR6LensLike -p "$COBAYA_PACKAGES_PATH"; then break; fi
  [[ "$i" == 4 ]] && { echo Q041_ACT_LENS_COBAYA_DATA_INSTALL_GATE=FAIL >&2; exit 2; }
  sleep 15
done
ACT_LENS_DATA_DIR="$COBAYA_PACKAGES_PATH/data/ACT_dr6_likelihood/v1.2"
test -d "$ACT_LENS_DATA_DIR"
test -n "$(find "$ACT_LENS_DATA_DIR" -type f -print -quit)"
echo "Q041_ACT_LENS_COBAYA_DATA_PATH_GATE=PASS path=$ACT_LENS_DATA_DIR"
...
rec={
 'q':'Q-041','program_id':'Q041-PLANCKPORT-V6','stage':'EXTERNAL_RUNTIME_PROVENANCE','status':'PASS',
...
pathlib.Path('q041_runtime/q041_external_runtime_provenance_v6.json').write_text(json.dumps(rec,indent=2,sort_keys=True)+'\n')
