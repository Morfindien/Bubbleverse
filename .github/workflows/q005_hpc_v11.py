...
def collect(cfg: dict[str, Any], model_name: str | None = None, restart: int | None = None) -> dict[str, Any]:
    ...
    summary = {
        ...
        "models": {},
        "provenance": {
            "config_sha256": sha256_file(ROOT / "q005_hpc_v11_config.yml"),
            "script_sha256": sha256_file(ROOT / "q005_hpc_v11.py"),
        },
    }
    names = (model_name,) if model_name else ("lcdm", "ede", "ede_local")
    for name in names:
...
def main() -> int:
    ...
    ap.add_argument("--config", default="q005_hpc_v11_config.yml")
