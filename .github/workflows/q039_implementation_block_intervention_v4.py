...
CAMSPEC_NATIVE_RELATIVE_CAL = ("cal0", "cal2")
HILLIPOP_NATIVE_RELATIVE_CAL = ("cal100A", "cal100B", "cal143A", "cal143B", "cal217A", "cal217B")
V3_FAILED_RUN_ID = 34140566884
V3_FAILED_JOB_ID = 101801510456
V3_FAILURE_CLASS = "TECHNICAL_PREFLIGHT_SCHEMA_RESOLUTION_FAILURE_NO_SCIENTIFIC_RESULT"
BASELINE = {
...
def _fixed_numeric_value(spec: Any) -> float | None:
    if isinstance(spec, Mapping):
        if _param_is_free(spec):
            return None
        if "value" in spec and finite(spec.get("value")):
...
def validate_camspec_native_relative_calibration_defaults(native: Mapping[str, Any]) -> dict[str, float]:
    """Validate the pinned CamSpec TT relative-calibration defaults.

    CamSpec v3.5.6 declares cal0=1 and cal2=1 in params_TT_CamSpec.yaml.
    Q032's raw info dictionary does not necessarily materialize component !defaults
    yet, so absence from raw info is not evidence that these parameters are absent
    from the likelihood schema.
    """
    if not isinstance(native, Mapping):
        raise RuntimeError("CAMSPEC_NATIVE_CALIBRATION_SCHEMA_GATE=FAIL not_mapping")
    out: dict[str, float] = {}
    for name in CAMSPEC_NATIVE_RELATIVE_CAL:
        if name not in native:
            raise RuntimeError(f"CAMSPEC_NATIVE_CALIBRATION_SCHEMA_GATE=FAIL missing_{name}")
        spec = native[name]
        if _param_is_free(spec):
            raise RuntimeError(f"CAMSPEC_NATIVE_CALIBRATION_SCHEMA_GATE=FAIL unexpectedly_free_{name}")
        val = _fixed_numeric_value(spec)
        if val is None or abs(val - 1.0) > 1e-12:
            raise RuntimeError(f"CAMSPEC_NATIVE_CALIBRATION_SCHEMA_GATE=FAIL {name}={spec!r}")
        out[name] = float(val)
    return out


def load_camspec_native_relative_calibration_defaults() -> tuple[dict[str, float], dict[str, Any]]:
    import importlib.metadata as importlib_metadata
    import cobaya  # type: ignore
    import yaml  # type: ignore

    version = importlib_metadata.version("cobaya")
    if version != "3.5.6":
        raise RuntimeError(f"CAMSPEC_COBAYA_VERSION_GATE=FAIL got={version}")
    package_root = Path(cobaya.__file__).resolve().parent
    path = package_root / "likelihoods" / "planck_NPIPE_highl_CamSpec" / "params_TT_CamSpec.yaml"
    if not path.is_file():
        raise RuntimeError(f"CAMSPEC_NATIVE_CALIBRATION_FILE_GATE=FAIL path={path}")
    data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    validated = validate_camspec_native_relative_calibration_defaults(data)
    return validated, {
        "source": "PINNED_COBAYA_3_5_6_COMPONENT_DEFAULTS",
        "component_file": str(path),
        "component_file_sha256": sha256_file(path),
        "cobaya_version": version,
        "resolution_semantics": "LIKELIHOOD_COMPONENT_DEFAULTS_AUDITED_BEFORE_EXPLICIT_NOOP_OVERRIDE",
    }


def apply_calibration_neutral(
    info: dict[str, Any], implementation: str,
    camspec_native_defaults_override: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    injected_native_default_overrides: list[str] = []
    native_default_provenance: dict[str, Any] | None = None

    if implementation == "camspec":
        if camspec_native_defaults_override is None:
            native, native_default_provenance = load_camspec_native_relative_calibration_defaults()
        else:
            native = validate_camspec_native_relative_calibration_defaults(camspec_native_defaults_override)
            native_default_provenance = {
                "source": "STATIC_TEST_OVERRIDE",
                "resolution_semantics": "TEST_ONLY_NOT_RUNTIME_PROVENANCE",
            }
        # The preregistered CamSpec intervention is a baseline no-op. Q032 raw
        # info is built before Cobaya expands likelihood !defaults, so explicit
        # native fixed-unity overrides are injected only after the pinned schema
        # proves cal0/cal2 are fixed to unity. This does not create new freedom.
        for name, neutral in native.items():
            if name in params:
                old = params[name]
                if _param_is_free(old):
                    raise RuntimeError(f"CAMSPEC_RELATIVE_CALIBRATION_BASELINE_GATE=FAIL free_{name}")
                val = _fixed_numeric_value(old)
                if val is None or abs(val - neutral) > 1e-12:
                    raise RuntimeError(f"CAMSPEC_RELATIVE_CALIBRATION_BASELINE_GATE=FAIL {name}={old!r}")
                already_neutral.append(name)
            else:
                params[name] = float(neutral)
                injected_native_default_overrides.append(name)
    elif implementation == "hillipop":
        missing = [n for n in HILLIPOP_NATIVE_RELATIVE_CAL if n not in params]
        if missing:
            raise RuntimeError(f"HILLIPOP_RELATIVE_CALIBRATION_GATE=FAIL missing={missing}")
        for name in HILLIPOP_NATIVE_RELATIVE_CAL:
            old = params[name]
            fixed = _fixed_numeric_value(old)
            if fixed is not None and abs(fixed - 1.0) <= 1e-12:
                already_neutral.append(name)
            else:
                changed.append(name)
            params[name] = 1.0
    else:
        raise RuntimeError(f"CALIBRATION_IMPLEMENTATION_GATE=FAIL implementation={implementation}")
    ...
    return {
        ...
        "injected_native_default_overrides": injected_native_default_overrides,
        "native_default_provenance": native_default_provenance,
        "camspec_explicit_override_is_documented_native_noop": implementation == "camspec",
...
def runtime_preflight(args: argparse.Namespace) -> int:
    ...
    # V4 runtime gate: instantiate the actual Cobaya model so component defaults,
    # parameter consumption and likelihood initialize hooks are exercised before
    # any expensive optimizer shard is released. No logposterior is evaluated.
    model = q32.create_model(info)
    model_initialized_without_optimizer = model is not None
    if not model_initialized_without_optimizer:
        raise RuntimeError("Q039_MODEL_INITIALIZATION_PREFLIGHT_GATE=FAIL")
    if arm_has_precision(args.arm):
        if not bool(precision.get("applied")):
            raise RuntimeError("Q039_PRECISION_RUNTIME_APPLY_GATE=FAIL")
        if int(precision.get("dimension", 0)) <= 0:
            raise RuntimeError("Q039_PRECISION_RUNTIME_DIMENSION_GATE=FAIL")
        intervention["precision_runtime_after_model_initialization"] = dict(precision)
    del model
    params = info.get("params")
    ...
    rec = {
        ...
        "scientific_preregister_program_id": SCIENTIFIC_PREREG_PROGRAM_ID,
        "stage": "Q039_RUNTIME_PREFLIGHT_V4", "status": "PASS",
        ...
        "model_initialized_without_optimizer": model_initialized_without_optimizer,
        "technical_model_initialization_only": True,
        "v3_failed_run_id": V3_FAILED_RUN_ID, "v3_failed_job_id": V3_FAILED_JOB_ID,
        "v3_failure_class": V3_FAILURE_CLASS,
