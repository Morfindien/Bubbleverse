#!/usr/bin/env python3
"""Q016 V14 static/runtime objective tests and final gate validation."""
...
def static_test(cfg_path,output):
    ...
    src=Path("q016_objective_reprofile_v14.py").read_text()
    agg=Path("q016_aggregate_v14.py").read_text()
    wf=Path(".github/workflows/q016-matched-cmb-surface-v14.yml").read_text() if Path(".github/workflows/q016-matched-cmb-surface-v14.yml").exists() else Path("q016-matched-cmb-surface-v14.yml").read_text()
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V14 objective.
    #
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v14_config.yml");p.add_argument("--output",required=True)
