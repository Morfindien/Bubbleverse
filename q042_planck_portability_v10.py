#!/usr/bin/env python3
"""Bubbleverse Q042 V10 — PolyChord + external-start Py-BOBYQA hard preflight.
...
RUN_ID="Q042-POLYCHORD-BOBYQA-PREFLIGHT-V10"
RESULT_ID="R-Q042-POLYCHORD-BOBYQA-PREFLIGHT-010"
...
def static_check(a:argparse.Namespace)->int:
    ...
    write_json(a.output,out);print("Q042_V10_STATIC_GATE=PASS");return 0
...
def prepare_nonoverlap(a:argparse.Namespace)->int:
    ...
    print("Q042_V10_NONOVERLAP_GATE=PASS");return rc
...
def runtime_preflight(a:argparse.Namespace)->int:
    ...
    cell_dir=Path(a.output).with_suffix("").parent/"q042_v10_cell_preflight";cell_dir.mkdir(parents=True,exist_ok=True)
    ...
    write_json(a.output,out);print("Q042_V10_RUNTIME_PREFLIGHT_GATE=PASS");return 0
...
def polychord_stage(a:argparse.Namespace)->int:
    ...
    try:
        ...
        print(f"Q042_V10_POLYCHORD_PROCESS_GATE=PASS action={a.action} arm={a.arm} model={a.model}")
        return 0
    finally:
        try:
...
def bobyqa_stage(a:argparse.Namespace)->int:
    ...
    print(f"Q042_V10_BOBYQA_PROCESS_GATE={'PASS' if rr['technical_ok'] else 'RECORDED_FAIL'} start={a.start_index} arm={a.arm} model={a.model}")
    return 0
...
def pilot_cell(a:argparse.Namespace)->int:
    ...
    fresh_json=outdir/"q042_polychord_fresh_v10.json"
    resume_json=outdir/"q042_polychord_resume_v10.json"
    ...
    for start_index in range(int(spec["bobyqa"]["pilot"]["external_starts_per_full_cell"])):
        bj=outdir/f"q042_bobyqa_start{start_index}_v10.json"
    ...
    write_json(outdir/"q042_pilot_result_v10.json",rec);print(f"Q042_V10_PILOT_GATE=PASS arm={a.arm} model={a.model}");return 0
...
def merge_pilots(a:argparse.Namespace)->int:
    ...
    rows=[]
    for p in Path(a.input_dir).rglob("q042_pilot_result_v10.json"):
        d=read_json(p)
    ...
    write_json(a.output,out);print("Q042_V10_PREFLIGHT_FINAL_GATE=PASS");return 0
