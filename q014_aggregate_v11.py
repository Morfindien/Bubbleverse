#!/usr/bin/env python3
"""Aggregate Q014 V11 Phase1+Phase2 same-run certification jobs."""
...
def aggregate(records,c):
    ...
    result={
      ...
      "program_status":"V11 SAME-RUN CROSS-MODE CERTIFICATION RESULTS AGGREGATED",
      ...
      "chains":{},
      "v10_numerical_parent":c["parent"]["v10"],
      "job_manifest":{
    ...
    for chain,spec in c["chains"].items():
        ...
        # Explicitly verify the same-run fixed->free embedded seed from Phase 2.
        ...
        if embedded and fix is not None:
            er=embedded[0]
            seed_obj=(er.get("seed_candidate") or {}).get("objective_chi2")
            emb_pass=(seed_obj is not None and math.isfinite(float(seed_obj))
                      and float(seed_obj) <= float(fix)+0.05
            ...
            cr["same_run_embedding"]={
                ...
                "final_fixed_best_objective":float(fix),
                "delta_seed_minus_fixed":float(seed_obj)-float(fix) if seed_obj is not None else None,
                "pass":emb_pass,
            }
        else:
    ...
    result["claim_boundary"]={
        ...
        "v10_not_interpreted_as_scientific_result":True,
    ...
    result["journal_update"]={
        ...
        "program":"q014_external_viability_v11.py",
        "workflow":"q014-external-viability-v11.yml",
        "validation_status":result["validation_status"],
        "what_was_computed":"V11 same-run two-phase external n=3 profile-deviance certification: V10-parent Phase1 refinement followed by Phase1-fixed/free seeded free-H0 Phase2 certification.",
        "scientific_consequence":"Interpret only after mandatory V11 validation passes; report each chain separately.",
        "what_remains_unresolved":[] if result["scientifically_usable_result"] else
            ["One or more primary V11 profiles remain numerically unstable, fail same-run embedding, or fail nesting monotonicity."],
