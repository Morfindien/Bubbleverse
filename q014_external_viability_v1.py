#!/usr/bin/env python3
"""Bubbleverse Q014 — n=3 EDE external likelihood viability V1.
...
CONFIG_NAME = "q014_external_viability_v1_config.yml"
CURRENT_Q = "Q014"
RUN_ID = "Q014-EXTERNAL-VIABILITY-V1"
CODE_VERSION = "1.0-external-chain-profile-separation"
...
def q011_exact_shared_vector(c: Mapping[str, Any], q011_path: str | Path) -> dict[str, float]:
    ...
    # IMPORTANT: all numbers below are extracted from the authoritative artifact.
    # Nothing is reconstructed from the rounded handoff.
    scope: Mapping[str, Any] = rec
    aliases = {
        "H0": ["H0", "h0"],
    ...
    vector = {name: extract_value(scope, names) for name, names in aliases.items()}
...
def run_task(chain: str, mode: str, start_family: str, restart_index: int,
    ...
    rsp = ROOT / "q014_v1_source_runtime" / "setup_imports.json"
    if rsp.exists():
        try:
...
def preflight(c: Mapping[str, Any], q011_path: str | Path | None = None,
    ...
    if require_environment:
        env: dict[str, Any] = {}
        for mod in ["cobaya", "classy", "candl", "spt_candl_data", "muse3glike"]:
            try:
                m = __import__(mod)
                env[mod] = {"pass": True, "file": getattr(m, "__file__", None)}
            except Exception as exc:
                env[mod] = {"pass": False, "error": repr(exc)}
        report["environment"] = env
        report["checks"]["EXTERNAL_ENVIRONMENT_GATE"] = all(v["pass"] for v in env.values())
...
def smoke(chain: str, q011_path: str | Path, output: str | Path, c: Mapping[str, Any]) -> int:
    ...
    try:
        ...
        report["status"] = "PASS" if finite else "FAIL"
    except Exception as exc:
...
def cli() -> int:
    ...
    if args.cmd == "preflight":
        r = preflight(c, args.q011, args.environment)
