...
def preflight(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    # FIX9: globality is provided by the external GitHub model x restart matrix.
    # Do not nest Py-BOBYQA restarts inside each independent matrix restart.
    bobyqa_user = cfg["sampler"]["minimize"]["override_bobyqa"]["user_params"]
    report["optimizer_restart_architecture_gate"] = (
        bobyqa_user.get("restarts.use_restarts") is False
        and bobyqa_user.get("restarts.use_soft_restarts") is False
        and bobyqa_user.get("restarts.auto_detect") is False
    )

    # FIX10: smoke must be deterministic and use the validated finite-domain point.
    sref = cfg.get("runtime_safety", {}).get("smoke_reference", {})
    try:
        scommon = sref["common"]
        sede = sref["ede"]
        sme = sref["varying_me"]
        snuis = sref["likelihood_nuisance"]
        report["deterministic_smoke_reference_gate"] = (
            sref.get("mode") == "DETERMINISTIC_FIXED_FINITE_POINT"
            and all(math.isfinite(float(scommon[k])) for k in
                    ("omega_b", "omega_cdm", "H0", "tau_reio", "n_s", "logA"))
            and math.isfinite(float(sede["fEDE"]))
            and float(sede["fEDE"]) >= ede_min
            and math.isfinite(float(sede["log10z_c"]))
            and math.isfinite(float(sede["thetai_scf"]))
            and math.isfinite(float(sme["varying_me"]))
            and all(math.isfinite(float(snuis[k])) for k in ("A_act", "P_act"))
        )
    except Exception:
        report["deterministic_smoke_reference_gate"] = False

    # FIX11: EDE shooting needs a smaller initial BOBYQA interpolation geometry.
    try:
        geom = cfg["runtime_safety"]["ede_optimizer_initial_geometry"]
        ede_rhobeg = float(geom["rhobeg"])
        ede_rhoend = float(cfg["sampler"]["minimize"]["override_bobyqa"]["rhoend"])
        report["ede_optimizer_geometry_gate"] = (
            math.isfinite(ede_rhobeg)
            and math.isfinite(ede_rhoend)
            and ede_rhobeg > ede_rhoend > 0.0
            and ede_rhobeg <= 1.0
        )
    except Exception:
        report["ede_optimizer_geometry_gate"] = False

    try:
        rp = cfg["runtime_safety"]["restart_result_policy"]
        min_valid = int(rp["minimum_valid_restarts"])
        min_fraction = float(rp["minimum_valid_fraction"])
        report["restart_result_policy_gate"] = (
            rp.get("coverage_requires_record_for_every_requested_restart") is True
            and rp.get("globality_uses_only_scientifically_valid_converged_results") is True
            and min_valid >= 1
            and 0.0 < min_fraction <= 1.0
        )
    except Exception:
        report["restart_result_policy_gate"] = False
...
def freeze_param_value(params: dict[str, Any], name: str, value: float) -> None:
    """Replace a sampled parameter by an exact fixed value for validation only."""
    old = params.get(name)
    keep: dict[str, Any] = {}
    if isinstance(old, dict) and "latex" in old:
        keep["latex"] = old["latex"]
    keep["value"] = float(value)
    params[name] = keep
...
def build_model(cfg: dict[str, Any], model_name: str, *, smoke: bool = False,
    ...
    if smoke:
        # FIX10: smoke is a deterministic infrastructure/finite-evaluation gate.
        # Run #18 proved that random ref draws can create false negatives for
        # EDE shooting. Use the already validated finite point from run #16.
        sref = cfg["runtime_safety"]["smoke_reference"]
        for k, val in sref["common"].items():
            if k in params and k not in ("A_s",):
                freeze_param_value(params, k, float(val))

        if kind == "ede":
            for k, val in sref["ede"].items():
                freeze_param_value(params, k, float(val))
        elif kind == "varying_me":
            for k, val in sref["varying_me"].items():
                freeze_param_value(params, k, float(val))

        frozen_nuisance = freeze_likelihood_nuisance_refs(likelihood)
        expected_nuisance = {
            k: float(v) for k, v in sref["likelihood_nuisance"].items()
        }
        if frozen_nuisance != expected_nuisance:
            raise RuntimeError(
                "Smoke nuisance reference mismatch: "
                f"actual={frozen_nuisance!r}, expected={expected_nuisance!r}"
            )
        ...
        for k, val in refs.items():
            if k in params and k not in ("A_s",):
                freeze_param_value(params, k, val)

        # FIX7: fixed-model comparison uses identical likelihood-local nuisances.
        frozen_nuisance = freeze_likelihood_nuisance_refs(likelihood)
    else:
        frozen_nuisance = {}
    ...
    if smoke or fixed_common:
        ...
        # FIX11:
        # Cobaya 3.5.6 otherwise starts Py-BOBYQA with rhobeg=1 in its affine,
        # covariance-scaled coordinates. EDE n=2 restart 1 in run #19 failed
        # after exactly 2*n+1=23 evaluations on a flat reject-penalty model.
        # Shrink only the INITIAL interpolation radius for scalar-field shooting
        # models. This does not change bounds/priors and the trust region may expand.
        if kind == "ede":
            sampler["minimize"]["override_bobyqa"]["rhobeg"] = float(
                cfg["runtime_safety"]["ede_optimizer_initial_geometry"]["rhobeg"]
            )
...
def parse_minimizer_failure(logfile: Path) -> dict[str, Any]:
    """Extract technical optimizer diagnostics without manufacturing a best fit.

    Py-BOBYQA's reported xmin is optimizer/scaled coordinates and is not
    promoted here to physical cosmological parameters. A MAXFUN point remains
    NON-CONVERGED and cannot pass a scientific result gate.
    """
    out: dict[str, Any] = {
        "logfile": str(logfile),
    ...
    if not logfile.exists():
        out["reason"] = "LOGFILE_MISSING"
        return out

    s = logfile.read_text(errors="replace")
    if "Maximum allowed objective evaluations reached" in s or "MAXFUN times" in s:
        out["status"] = "OPTIMIZER_MAXFUN_EXHAUSTED"
    elif (
        "Singular matrix in mini-model interpolation" in s
        or "Exit flag = -3" in s
        or "interpolation points produced a singular linear system" in s
    ):
        out["status"] = "OPTIMIZER_INTERPOLATION_LINALG_FAILURE"

    m = re.search(r"Objective value f\(xmin\)\s*=\s*([-+0-9.eE]+)", s)
    if m:
        out["objective_best_seen"] = float(m.group(1))

    m = re.search(r"Needed\s+(\d+)\s+objective evaluations", s)
    if m:
        out["objective_evaluations"] = int(m.group(1))

    m = re.search(r"Did a total of\s+(\d+)\s+runs", s)
    if m:
        out["internal_pybobyqa_runs"] = int(m.group(1))

    m = re.search(
        r"Approximate gradient\s*=\s*\[(.*?)\]\s*Approximate Hessian",
        s,
        re.S,
    )
    if m:
        try:
            grad = [float(x) for x in m.group(1).replace("\n", " ").split()]
            out["approximate_gradient"] = grad
            out["approximate_gradient_l2"] = sum(x*x for x in grad) ** 0.5
        except Exception:
            pass

    if out["status"] == "OPTIMIZER_MAXFUN_EXHAUSTED":
        out["classification"] = "TECHNICAL_NUMERICAL_NOT_SCIENTIFIC"
        out["reason"] = (
            "Optimizer exhausted MAXFUN before Cobaya accepted convergence. "
            "No one-point best fit was written."
        )
    elif out["status"] == "OPTIMIZER_INTERPOLATION_LINALG_FAILURE":
        out["classification"] = "TECHNICAL_NUMERICAL_NOT_SCIENTIFIC"
        out["reason"] = (
            "Py-BOBYQA interpolation geometry became singular before a valid "
            "converged one-point result was written."
        )
        obj = out.get("objective_best_seen")
        grad = out.get("approximate_gradient_l2")
        out["reject_plateau_suspected"] = bool(
            isinstance(obj, (int, float))
            and math.isfinite(float(obj))
            and float(obj) >= 1e20
            and isinstance(grad, (int, float))
            and float(grad) <= 1e-12
        )
        out["low_ee_rejection_seen"] = (
            "low EE multipole out of range, rejecting point" in s
        )
    return out


def collect(cfg: dict[str, Any], model: str, restart: int) -> dict[str, Any]:
    # FIX13: result collection is itself part of failure provenance. It must work
    # even if setup failed before Cobaya started and therefore before any output
    # directory was created.
    results_dir = ROOT / cfg["results_dir"]
    results_dir.mkdir(parents=True, exist_ok=True)
    prefix = results_dir / f"{model}_r{restart}"
    ...
    if p is None:
        logfile = ROOT / cfg["logs_dir"] / f"q005_v14_{model}_r{restart}.log"
        setup_failure_path = results_dir / "q005_setup_failure.json"

        # If the scientific run never started, prefer the upstream setup record
        # over an invented optimizer diagnosis.
        if not logfile.exists() and setup_failure_path.exists():
            try:
                setup_failure = json.loads(setup_failure_path.read_text(encoding="utf-8"))
            ...
            result["reason"] = "UPSTREAM_SETUP_FAILURE_NO_SCIENTIFIC_RUN"
            result["technical_status"] = "SETUP_FAILED"
            result["scientific_status"] = "NO_SCIENTIFIC_RESULT"
            result["setup_failure"] = setup_failure
        else:
            diag = parse_minimizer_failure(logfile)
            result["optimizer_diagnostic"] = diag
            if diag.get("status") == "OPTIMIZER_MAXFUN_EXHAUSTED":
                result["reason"] = "OPTIMIZER_MAXFUN_NO_VALID_ONEPOINT"
                result["technical_status"] = "OPTIMIZER_NOT_CONVERGED"
                result["scientific_status"] = "NO_SCIENTIFIC_RESULT"
            elif diag.get("status") == "OPTIMIZER_INTERPOLATION_LINALG_FAILURE":
                result["reason"] = "OPTIMIZER_INTERPOLATION_LINALG_NO_VALID_ONEPOINT"
                result["technical_status"] = "OPTIMIZER_GEOMETRY_FAILURE"
                result["scientific_status"] = "NO_SCIENTIFIC_RESULT"
            elif not logfile.exists():
                result["reason"] = "UPSTREAM_EXECUTION_NOT_RUN_OR_LOG_MISSING"
                result["technical_status"] = "UPSTREAM_EXECUTION_MISSING"
                result["scientific_status"] = "NO_SCIENTIFIC_RESULT"
            else:
    ...
    out = results_dir / f"q005_job_result_{model}_r{restart}.json"
