...
Q='Q-041'; PID='Q041-PLANCKPORT-V13'; RID='R-Q041-EDE-DOWNSTREAM-PORTABILITY-013'
...
def static(a):
    ...
    assert ent['q']==Q and ent['status']=='ACTIVE' and ent['workflow_id']=='q041-planck-portability-v13.yml'
    assert ent['scientific_program']=='q041_planck_portability_v13.py'
    wf=pathlib.Path(a.workflow).read_text(encoding='utf-8')
    for token in [PID,'q041_prepare_frozen_core_v13.sh','prepare-nonoverlap','runtime-preflight','P_A6_L6_D2','P_L6_D2','P_A6_D2','P_A6_L6','aggregate']:
    ...
    setup=pathlib.Path('q041_setup_v13.sh').read_text(encoding='utf-8')
    core=pathlib.Path('q041_prepare_frozen_core_v13.sh').read_text(encoding='utf-8')
    ...
    assert 'Q041_DESI_DR2_BAO_DTYPE_COMPATIBILITY_PATCH_GATE=PASS' in setup
    assert 'Q041_DESI_DR2_BAO_DTYPE_PATCH_DIAGNOSTIC_BEGIN' in setup
    assert 'compile(verify, str(p), "exec")' in setup
    assert "literal backslash-n" not in setup
    assert "old='            self.has_type" not in setup
    assert 'Q041_DESI_DR2_BAO_STRINGDTYPE_RUNTIME_GATE=PASS' in setup
    assert 'type_last_col.__class__ == pd.StringDtype' in setup
    assert 'b76b6fed2a6c8c5594c6f92d5058bef10079746a' in setup
    assert 'Restore prior V11 full environment cache' in wf
    assert 'desi_bao_all.is_installed(path=packages, show_error=True) is True' in setup
    assert 'q041_chain_metadata_v13.json' in program
    assert 'q041_chain_metadata_v7.json' not in program
    assert program.count('q041_chain_metadata_v13.json') == 2
    ...
    assert 'name: q041-chain-${{ matrix.implementation }}-${{ matrix.combo }}-c${{ matrix.chain }}-v13' in wf
    assert 'pattern: q041-chain-*-v13' in wf
    ...
    assert 'q041-v13-q032core-' in wf
    ...
    current_runtime_refs = [
        ...
        'q041_planck_portability_tests_v13.py',
        'q041_planck_portability_preregister_v13.json',
        'q041_planck_portability_source_lock_v13.json',
        'q041_setup_v13.sh',
        'q041_prepare_frozen_core_v13.sh',
        'q041-static-v13',
        'q041-environment-v13',
        'q041-chain-*-v13',
        'q041-final-v13',
    ]
    ...
    stale_runtime_patterns = [
        r'q041_planck_portability_tests_v(?:[3-9]|10|11)\\.py',
        r'q041_planck_portability_preregister_v(?:[3-9]|10|11)\\.json',
        r'q041_planck_portability_source_lock_v(?:[3-9]|10|11)\\.json',
        r'q041_planck_portability_v(?:[3-9]|10|11)\\.py',
        r'q041_setup_v(?:[3-9]|10|11)\\.sh',
        r'q041_prepare_frozen_core_v(?:[3-9]|10|11)\\.sh',
        r'q041-(?:static|environment|final)-v(?:[3-9]|10|11)\\b',
        r'q041-chain-[^\\n]*-v(?:[3-9]|10|11)\\b',
    ]
