#!/usr/bin/env python3
"""
Bubbleverse Q011 — n=3 EDE fixed-H0 globality / basin certification V4.

V4 is a technical-gate semantics patch layered on the repaired Q011 orchestration over Q011 V1. It deliberately preserves the
...
DEFAULT_CONFIG = "q011_ede_globality_certification_v4_config.yml"
CODE_VERSION = "4.0-q011-330min-best-observed-cutoff"
CURRENT_Q = "Q011"

# Ensure V1 aggregate/result helpers record V4 program identity when delegated.
...
def run_optimizer(
    ...
    """
    V4 orchestration-equivalent replacement for V1 run_optimizer.
    ...
    # V4 FIX-004: the workflow gives the optimizer an internal 320-minute
    # cutoff inside a 330-minute GitHub job. SIGINT is treated as a harvest
    # request, not as a scientific failure. We then use any finite Cobaya
    # one-point/best point that already exists.
    base = v1.base_cfg(c)
    wallclock_cutoff = False
    try:
        rc = core.cobaya_run(rendered, base)
    except KeyboardInterrupt:
        rc = 130
        wallclock_cutoff = True
    ...
    rec: dict[str, Any] = {
        ...
        "cobaya_exit": int(rc),
        "wallclock_cutoff": bool(wallclock_cutoff),
        "result_policy": "USE_ANY_FINITE_BEST_POINT_REGARDLESS_OF_CONVERGENCE_EXIT",
        "status": "FAIL",
        "scientific_surface": "FROZEN_Q005_V14",
        "v4_inherited_repairs": [
    ...
    if one is not None:
        ...
        if ok:
            ...
            rec["status"] = "PASS"
            rec["convergence_status"] = (
                "CONVERGED" if int(rc) == 0
                else "TIME_LIMIT_BEST_OBSERVED" if wallclock_cutoff or int(rc) in (124, 130, 137, 143)
                else "NONZERO_EXIT_BEST_OBSERVED"
            )
            rec["accepted_despite_nonzero_exit"] = bool(int(rc) != 0)
        else:
            rec["reason"] = reason
    else:
...
def technical_gate(c: dict[str, Any], q008: dict[str, Any], output: str) -> int:
    ...
    obj["output"] = str(
        (ROOT / c["paths"]["results"] / "cobaya_q011_v4_technical_gate").resolve()
    )
    rendered = ROOT / c["paths"]["rendered"] / "q011_v4_technical_gate.yaml"
    ...
    copied_log = ROOT / c["paths"]["results"] / "q011_v4_technical_gate_cobaya.log"
...
def main() -> int:
    ...
    if a.cmd == "preflight":
        ...
        result["v4_patch_gate"] = (
            c["project"]["run_id"] == "Q011-EDE-GLOBALITY-CERTIFICATION-V4"
        )
        result["status"] = "PASS" if (
            result["status"] == "PASS" and result["v4_patch_gate"]
        ) else "FAIL"
    ...
    if v1.preflight(c, q8)["status"] != "PASS":
        raise SystemExit("Q011_V4_PREFLIGHT_GATE=FAIL")
