...
Q007_CODE_VERSION = "2.0-h0-fixed"
...
def evaluate_point(c, q006_path, model_name, target_h0):
    q6 = load_json(q006_path)
    profiles = find_q006_profiles(q6)

    requested_h0 = float(target_h0)
    key = (model_name, round(requested_h0, 6))

    if key not in profiles:
        raise RuntimeError(f"Q006_PROFILE_GATE=FAIL missing={key}")

    profile = profiles[key]

    # Q-006 is a fixed-H0 profile calculation. H0 can therefore be absent
    # from Cobaya's serialized bestfit sampled-parameter block.
    if "target_h0" not in profile:
        raise RuntimeError("Q006_FIXED_H0_GATE=FAIL missing_parent_target_h0")

    parent_h0 = float(profile["target_h0"])

    # Never infer/interpolate H0: require the requested point to match
    # the immutable Q-006 profile metadata exactly.
    if not math.isclose(parent_h0, requested_h0, rel_tol=0.0, abs_tol=1e-9):
        raise RuntimeError(
            f"Q006_FIXED_H0_GATE=FAIL requested={requested_h0} parent={parent_h0}"
        )

    # Build once to learn the exact sampled parameter set.
    core = import_core()
    base = core.load_cfg(c["parent"]["base_config"])
    tmp = core.build_model(base, model_name, smoke=False, restart=0)
    needed = sampled_names(tmp)

    # All sampled parameters except H0 must come explicitly from the exact
    # Q-006 bestfit record. H0 alone comes from Q-006 target_h0 metadata.
    needed_from_bestfit = set(needed) - {"H0"}
    vals = extract_param_vector(profile["bestfit"], needed_from_bestfit)
    vals["H0"] = parent_h0

    missing = sorted(set(needed) - set(vals))
    if missing:
        raise RuntimeError(
            f"Q006_EXACT_POINT_GATE=FAIL missing_sampled_parameters={missing}"
        )

    print(
        f"Q007_CODE_VERSION={Q007_CODE_VERSION} "
        f"Q006_FIXED_H0_GATE=PASS model={model_name} H0={parent_h0}"
    )
    ...
    result = {
        ...
        "code_version": Q007_CODE_VERSION,
        "model": model_name,
        "target_h0": requested_h0,
        "parent_target_h0": parent_h0,
        "q006_fixed_h0_gate": "PASS",
        "q006_point_sha256": sha256_json(vals),
    ...
    # Preserve original Q-006 component record for provenance.
    result["q006_delta_component_record"] = profile.get(
        "delta_chi2_components", {}
    )
