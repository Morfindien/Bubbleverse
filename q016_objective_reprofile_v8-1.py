#!/usr/bin/env python3
"""Bubbleverse Q016 V8 exact-objective fixed/free reprofile.

V8 changes only the computational objective implementation and continuation logic.
...
def load_phase1(path):
    ...
    if d.get("q")!=Q or d.get("run_id")!=RUN or d.get("stage")!="PHASE1_FIXED_SUMMARY":
        raise RuntimeError("PHASE1_SUMMARY_GATE=FAIL identity")
    if d.get("status")!="PASS":
        raise RuntimeError("PHASE1_SUMMARY_GATE=FAIL status")
    return d

def source_vector(info,branch,mode,family,cfg,phase1_path=None):
    ...
    if family.startswith("parent_fixed"):
        ...
        source={"kind":"SAME_RUN_PHASE1_FIXED_BEST",
...
def run_task(args):
    ...
    status={
        ...
        "parent":{"github_run_id":33732688762,"head_sha":"d14880138ebb537173e49417b537a68454d53602",
                  "run_id":"Q016-MATCHED-CMB-SURFACE-V6","role":"NUMERICAL_SEED_PARENT_ONLY"}
    }
    ...
    raw_source,source_meta,applied=source_vector(
        info,branch,mode,family,cfg,args.phase1_summary)
    ...
    status.update({"status":"RUNNING","seed_candidate":seed,"source_vector_meta":source_meta,
                   "runtime_provenance_file":f"q016_v8_source_runtime/setup_{branch}.json"})
    write_json(out,status)
...
def main():
    ...
    ap.add_argument("--config",default=str(ROOT/"q016_matched_cmb_surface_v8_config.yml"))
