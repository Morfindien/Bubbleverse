#!/usr/bin/env python3
"""Bubbleverse Q014 V9 — V8 basin-replication numerical certification.

V9 preserves the V8 scientific surface and normalization-free objective exactly.
...
Planck/NPIPE approximation. V9 does not relax the <=1.0 stability requirement.

For each free/fixed profile V9 launches four deterministic continuations:
...
CONFIG_NAME="q014_external_viability_v9_config.yml"
RUN_ID="Q014-EXTERNAL-VIABILITY-V9"
...
def start_vector(chain:str, family:str, c:Mapping[str,Any], info:Mapping[str,Any], q011_vec:Mapping[str,float])->dict[str,float]:
    if family=="q011": return _original_start(chain,family,c,info,q011_vec)
    families={
    ...
    if family not in families: raise KeyError(f"Unknown start family {family}")
...
def run_task_v9(chain,mode,family,restart,q011_path,v8_path,output,c,max_evals=None)->int:
    ...
    try: seed=seed_evaluation(chain,mode,family,restart,q011_path,c,out)
        ...
        impl.dump_json(out,{"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN_ID,"chain":chain,"mode":mode,
            "start_family":family,"restart_index":restart,"status":"TECHNICAL_FAILURE","failure_type":"V9_SEED_OR_PRIOR_GATE",
    ...
    if rec.get("status")=="COMPLETE":
        ...
        tol=float(c["execution"]["v9_seed_policy"]["seed_objective_tolerance"])
