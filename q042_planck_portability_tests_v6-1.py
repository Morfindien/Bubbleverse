...
def static(a):
 ...
 gates={
  ...
  "WORKFLOW_IDENTITY":"PROGRAM_ID: Q042-PREFLIGHT-V6" in w and "CURRENT_Q: Q-042" in w,
  ...
  "V5_FAILURE_PRESERVED":l.get("technical_parent",{}).get("program_id")=="Q042-PREFLIGHT-V5" and l.get("technical_parent",{}).get("scientific_result")=="NONE",
  ...
  "WORKFLOW_SELF_REFERENCE":"--workflow .github/workflows/q042-planck-portability-v6.yml" in w,
  "PILOT_RESULT_VERSION":"q042_pilot_result_v6.json" in p and "q042_pilot_result_v4.json" not in p and "q042_pilot_result_v5.json" not in p,
  "REGISTRY_ARTIFACT_VERSION":all(str(x).endswith("v6") or "*-v6" in str(x) for x in entry.get("result_artifacts",[])) and len(entry.get("result_artifacts",[]))==4,
  "REGISTRY_ENTRY":entry.get("q")==Q and entry.get("status")=="ACTIVE" and entry.get("workflow_id")=="q042-planck-portability-v6.yml",
 ...
 print("Q042_V6_STATIC_TEST_GATE=PASS")
def final(a):
 ...
 if status!="PASS":raise SystemExit(2)
 print("Q042_V6_FINAL_TEST_GATE=PASS")
def parser():
