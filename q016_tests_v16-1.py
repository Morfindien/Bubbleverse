#!/usr/bin/env python3
"""Q016 V16 static/runtime objective tests and final gate validation."""
import argparse,ast,json,math
from pathlib import Path
import yaml

RUN="Q016-MATCHED-CMB-SURFACE-V16"
RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-016"
...
def workflow_job_block(workflow_text, job_name):
    token=f"  {job_name}:\n"
    start=workflow_text.find(token)
    if start<0:
        raise RuntimeError(f"WORKFLOW_JOB_BLOCK_GATE=FAIL {job_name}")
    # Find the next top-level (two-space) job declaration.
    pos=start+len(token)
    next_pos=len(workflow_text)
    for candidate in ("  prepare:\n","  import_v10_parent:\n","  planck_extension:\n","  merge_primary:\n","  primary_endpoint_gates:\n"):
        q=workflow_text.find(candidate,pos)
        if q>=0 and q<next_pos:
            next_pos=q
    return workflow_text[start:next_pos]

def static_test(cfg_path,output):
    c=yaml.safe_load(Path(cfg_path).read_text())
    src=Path("q016_objective_reprofile_v16.py").read_text()
    agg=Path("q016_aggregate_v16.py").read_text()
    wf=Path(".github/workflows/q016-matched-cmb-surface-v16.yml").read_text() if Path(".github/workflows/q016-matched-cmb-surface-v16.yml").exists() else Path("q016-matched-cmb-surface-v16.yml").read_text()
    ...
    checks={
        ...
        "EXTENSION_FAMILY_CONFIG_GATE":
            c["execution"]["continuation"]["phase2_families"]==[
                "v10_planck_free_best_jitter_plus",
                "v10_planck_free_best_jitter_minus",
                "v10_planck_free_best_cross_a",
                "v10_planck_free_best_cross_b",
            ],
        "NONZERO_PERTURBATION_GATE":
            "family.startswith(\"v10_planck_free_best_\")" in src and
            "patterned_jitter" in src and
            "CROSS_A_SIGNS" in src and
            "CROSS_B_SIGNS" in src and
            "NONZERO_PERTURBATION_GATE" in src,
        "RECURSIVE_PARENT_DISCOVERY_GATE":".rglob(" in agg,
        "IMPORT_PARENT_PYYAML_DEPENDENCY_GATE":
            "python -m pip install PyYAML==6.0.2" in workflow_job_block(wf,"import_v10_parent"),
        "PRIMARY_GATES_PYYAML_DEPENDENCY_GATE":
            "python -m pip install PyYAML==6.0.2" in workflow_job_block(wf,"primary_endpoint_gates"),
        "PREPARE_TEST_DEPENDENCY_GATE":
            "python -m pip install -r q005_hpc_v14_requirements.txt" in workflow_job_block(wf,"prepare"),
        "FROZEN_Q014_V10_BLOB_REFERENCE_GATE":
            'q014_external_viability_v10.py' in wf and
            '9085bdea8e5732b5e4c7095068d284918157948d' in wf,
        "FROZEN_Q014_V12_BLOB_REFERENCE_GATE":
            'q014_external_viability_v12.py' in wf and
            'd2c8ba19aed63d8e1b17a0ef4e18e2dd6981a599' in wf,
        "FROZEN_Q014_V12_CONFIG_REFERENCE_GATE":
            'q014_external_viability_v12_config.yml' in src,
        "FORBID_Q014_REVISION_LEAK_GATE":
            all(x not in (wf+"\n"+src) for x in (
                'q014_external_viability_v13.py',
                'q014_external_viability_v14.py',
                'q014_external_viability_v15.py',
                'q014_external_viability_v13_config.yml',
                'q014_external_viability_v14_config.yml',
                'q014_external_viability_v15_config.yml',
            )),
        "FROZEN_Q005_REQUIREMENTS_GATE":
            'q005_hpc_v14_requirements.txt' in wf and
            'q005_hpc_v15_requirements.txt' not in wf and
            'q005_hpc_v16_requirements.txt' not in wf,
        "CURRENT_SEED_LOCK_POINTER_GATE":
            c["execution"]["continuation"]["seed_lock_file"]=="q016_v6_objective_seed_lock_v16.json",
        "CURRENT_V10_PARENT_LOCK_POINTER_GATE":
            c["execution"]["continuation"]["v10_primary_parent_lock_file"]=="q016_v10_primary_parent_lock_v16.json",
        "FORBID_STALE_SEED_LOCK_POINTER_GATE":
            all(x not in str(c["execution"]["continuation"].get("seed_lock_file","")) for x in (
                "_v11.json","_v12.json","_v13.json","_v14.json","_v15.json"
            )),
        "V10_PLANCK_LAZY_V6_LOCK_GATE":
            'lock=None' in src and
            'if family.startswith("parent_fixed"):' in src and
            'elif family.startswith("v10_planck_free_best_"):' in src,
        "WORKFLOW_DISPLAY_IDENTITY_GATE":
            wf.startswith("name: Bubbleverse Q016 Matched CMB Surface V16"),
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V16 objective.
    #
    ...
    # the target lay ~7 sigma outside Cobaya's allowed numerical box. V14 instead
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v16_config.yml");p.add_argument("--output",required=True)
