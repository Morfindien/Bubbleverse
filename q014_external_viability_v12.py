#!/usr/bin/env python3
"""Bubbleverse Q014 V12 — fixed-to-free H0 embedding repair + same-run certification.

V12 preserves the frozen Q014 V5 likelihood/model surface and the
normalization-free objective introduced in V8. V11 completed all 30 optimizer
jobs and fixed the earlier cross-mode monotonicity problem, but remained
numerically unresolved because:
  * Planck free-H0 best-two spread was 1.0744961952 (> 1.0),
  * Planck fixed-H0 best-two spread was 1.5114935186 (> 1.0), and
  * the Phase-2 fixed->free embedding seed was constructed with the default H0
    because the code only set H0=71.5 when H0 was absent from the base vector.

V12 repairs that exact bug by unconditionally setting H0=71.5 whenever a
fixed-H0 parent minimum is embedded into another profile. Phase 2 also
re-certifies fixed minima from the same-run Phase-1 best points so that both
Planck free and fixed profiles must independently reproduce within the
unchanged <=1.0 stability gate.

No scientific likelihood, dataset, parameter bound, objective basis, or
validation threshold is relaxed. V11 is a numerical parent only.
"""
...
RUN_ID = "Q014-EXTERNAL-VIABILITY-V12"
RESULT_ID = "R-Q014-EDE-EXTERNAL-VIABILITY-009"
CODE_VERSION = "12.0-fixed-to-free-h0-embedding-repair"
V11_EXPECTED_RUN = "Q014-EXTERNAL-VIABILITY-V11"
V11_EXPECTED_RESULT = "R-Q014-EDE-EXTERNAL-VIABILITY-008"
BASIS = "SCALAR_TOTAL_LIKELIHOOD_CHI2_PLUS_NORMALIZATION_FREE_GAUSSIAN_CONSTRAINT_SHAPES"

V11_RESULT_PATH: Path | None = None
...
def load_v11_result(path: str | Path | None = None) -> dict[str, Any]:
    p = Path(path) if path is not None else V11_RESULT_PATH
    if p is None:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL no V11 result path")
    d = json.loads(Path(p).read_text(encoding="utf-8"))
    if d.get("q") != "Q014" or d.get("run_id") != V11_EXPECTED_RUN or d.get("result_id") != V11_EXPECTED_RESULT:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL identity")
    if d.get("scientifically_usable_result") is not False or d.get("validation_status") != "NUMERICALLY UNRESOLVED":
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL status")
    if d.get("objective_basis") != BASIS:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL objective basis")
    jm = d.get("job_manifest", {})
    if int(jm.get("expected", -1)) != 30 or int(jm.get("returned", -2)) != 30:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL completeness")
    if int(jm.get("phase1_expected", -1)) != 18 or int(jm.get("phase2_expected", -1)) != 12:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL phase completeness")
    if jm.get("missing") or jm.get("duplicates") or jm.get("runtime_source_compatible") is not True:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL provenance")
    # Preserve the exact V11 unresolved signature that motivates V12.
    pc = d["chains"]["planck_npipe_k039_approx"]
    if pc["profiles"]["reference_free_h0"].get("stable") is True:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL Planck free unexpectedly stable")
    if pc["profiles"]["fixed_h0_71p5"].get("stable") is True:
        raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL Planck fixed unexpectedly stable")
    for name in ("planck_npipe_k039_approx", "spt_d1_only"):
        if d["chains"][name]["penalties"].get("fixed_profile_monotonicity_pass") is not True:
            raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL monotonicity signature")
        if d["chains"][name].get("same_run_embedding", {}).get("pass") is not False:
            raise RuntimeError("V11_NUMERICAL_PARENT_GATE=FAIL embedding signature")
    return d
...
def v11_best_minimum(chain: str, mode: str) -> dict[str, float]:
    return best_minimum_from_result(load_v11_result(), chain, mode)
...
def bounded_jitter(base: dict[str, float], info: Mapping[str, Any], sign: float) -> dict[str, float]:
    # Reuse the already audited V10/V11 bounded-jitter implementation exactly.
...
def start_vector(chain: str, family: str, c: Mapping[str, Any], info: Mapping[str, Any],
    ...
    # Phase 1: V11 numerical-parent minima.
    v11_families = {
        "v11_free_best": ("reference_free_h0", False, +1.0),
        "v11_free_jitter": ("reference_free_h0", True, +1.0),
        "v11_fixed_best": ("fixed_h0_71p5", False, -1.0),
        "v11_fixed_jitter": ("fixed_h0_71p5", True, -1.0),
    }
    ...
    if family in v11_families:
        source_mode, jitter, sign = v11_families[family]
        base = impl.q005_default_reference(info)
        base.update(v11_best_minimum(chain, source_mode))
        if source_mode == "fixed_h0_71p5":
            # V12 FIX: fixed-H0 minima do not serialize H0 in the minimum row.
            # q005_default_reference already contains an H0, so testing for absence
            # is wrong. Always overwrite with the exact fixed target.
    ...
    if family in p1_families:
        ...
        if source_mode == "fixed_h0_71p5":
            # Mathematically exact same-run embedding of the fixed point into
            # the free-H0 space at H0=71.5. This override is unconditional.
            base["H0"] = float(c["scientific_surface"]["target_h0"])
        return bounded_jitter(base, info, sign) if jitter else base
...
def run_task_v12(chain: str, mode: str, family: str, restart: int, q011_path: str | Path,
                 v11_path: str | Path, phase1_summary: str | Path | None,
                 output: str | Path, c: Mapping[str, Any], max_evals=None) -> int:
    global V11_RESULT_PATH, PHASE1_SUMMARY_PATH
    V11_RESULT_PATH = Path(v11_path)
    load_v11_result()
    ...
    try:
        ...
        dump_json(out, {
            ...
            "status": "TECHNICAL_FAILURE", "failure_type": "V12_SEED_OR_PRIOR_GATE",
            "error": repr(exc), "finite_result": False,
        })
        return 9
    rc = impl.run_task(chain, mode, family, restart, q011_path, out, c, max_evals)
    rec = json.loads(out.read_text(encoding="utf-8"))
    rec["v11_numerical_parent"] = {
        "run_id": V11_EXPECTED_RUN, "result_id": V11_EXPECTED_RESULT,
        "scientifically_usable_result": False,
    }
    rec["continuation_phase"] = (
        "PHASE2_SAME_RUN_FREE_CERTIFICATION" if family.startswith("p1_") and mode == "reference_free_h0" else
        ("PHASE2_SAME_RUN_FIXED_REPLICATION" if family.startswith("p1_") else "PHASE1_V11_CONTINUATION")
    )
    ...
    rec["phase1_source_objective_chi2"] = None
    if family.startswith("p1_"):
        source_mode = "fixed_h0_71p5" if family.startswith("p1_fixed") else "reference_free_h0"
        rec["phase1_source_objective_chi2"] = float(load_phase1_summary()["chains"][chain][source_mode]["best_objective_chi2"])
...
def v11_parent_preflight(path: str | Path) -> dict[str, Any]:
    try:
        d = load_v11_result(path)
        ok, err = True, None
    except Exception as exc:
    ...
    return {
        ...
        "planck_fixed_spread": d.get("chains", {}).get("planck_npipe_k039_approx", {}).get("profiles", {}).get("fixed_h0_71p5", {}).get("best_two_delta_chi2"),
        "spt_only_monotonicity": d.get("chains", {}).get("spt_d1_only", {}).get("penalties", {}).get("fixed_profile_monotonicity_pass"),
        "same_run_embedding": d.get("chains", {}).get("spt_d1_only", {}).get("same_run_embedding", {}).get("pass"),
    }
...
def cli() -> int:
    ...
    p.add_argument("--v11-result", required=True)
    ...
    p.add_argument("--start-family", choices=[
        "v11_free_best", "v11_free_jitter", "v11_fixed_best", "v11_fixed_jitter",
    ...
    if a.cmd == "plan":
        ...
        return 0
    if a.cmd == "preflight":
        global V11_RESULT_PATH
        V11_RESULT_PATH = Path(a.v11_result)
        r = impl.preflight(c, a.q011, a.environment, a.chain)
        vp = v11_parent_preflight(a.v11_result)
        r["checks"]["V11_NUMERICAL_PARENT_GATE"] = vp["pass"]
        r["v11_parent"] = vp
        ...
        r["checks"]["SAME_RUN_FIXED_TO_FREE_DESIGN_GATE"] = c["execution"]["v12_same_run_policy"]["same_run_fixed_to_free_embedding_required"] is True
    ...
    if a.cmd == "run":
        return run_task_v12(a.chain, a.mode, a.start_family, a.restart_index, a.q011,
                            a.v11_result, a.phase1_summary, a.output, c, a.max_evals)
    return 2
