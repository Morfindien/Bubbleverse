...
DEFAULT_CONFIG = "q009_act_corrected_minima_attribution_v8_config.yml"
CODE_VERSION = "8.0-q007-fixed-vector-consumption-fixed"
...
def q007_baseline_point(c: dict[str, Any], q007_base: Any) -> dict[str, Any]:
    if q007_base.get("q") != "Q-007":
        raise RuntimeError("Q007_BASELINE_POINT_GATE=FAIL wrong_q")
    if q007_base.get("model") != "ede_n3":
        raise RuntimeError("Q007_BASELINE_POINT_GATE=FAIL wrong_model")
    if q007_base.get("status") != "PASS":
        raise RuntimeError("Q007_BASELINE_POINT_GATE=FAIL status_not_pass")

    expected_h0 = float(c["historical_baseline"]["h0"])
    actual_h0 = q007_base.get("target_h0")
    if not isinstance(actual_h0, (int, float)) or not math.isclose(
        float(actual_h0), expected_h0, rel_tol=0.0, abs_tol=1e-6
    ):
        raise RuntimeError(
            f"Q007_BASELINE_POINT_GATE=FAIL H0={actual_h0} expected={expected_h0}"
        )

    vals = q007_base.get("fixed_sampled_parameters")
    act = q007_base.get("act")
    if not isinstance(vals, dict):
        raise RuntimeError("Q007_BASELINE_POINT_GATE=FAIL missing_fixed_sampled_parameters")

    required_baseline_params = {
        "omega_b", "omega_cdm", "tau_reio", "n_s", "logA",
        "fEDE", "log10z_c", "thetai_scf", "A_act", "P_act", "H0",
    }
    missing_baseline_params = sorted(required_baseline_params - set(vals))
    if missing_baseline_params:
        raise RuntimeError(
            "Q007_BASELINE_POINT_GATE=FAIL "
            f"missing_fixed_sampled_parameters={missing_baseline_params}"
        )

    if not isinstance(act, dict):
        raise RuntimeError("Q007_BASELINE_POINT_GATE=FAIL missing_act")
    for field in (
        "chi2_act_reconstructed",
        "by_spectrum_signed_fullcov",
        "by_band_signed_fullcov",
    ):
        if field not in act:
            raise RuntimeError(f"Q007_BASELINE_POINT_GATE=FAIL missing_act_{field}")
...
def q007_historical(q007: Any, target_h0: float) -> dict[str, Any]:
    ...
    if not isinstance(profiles, list):
        raise RuntimeError(
            "Q007_COMPARISON_GATE=FAIL expected_models.ede_n3.profiles_list"
        )
    ...
    for rec in profiles:
        if not isinstance(rec, dict):
            continue
        h0 = rec.get("H0", rec.get("target_h0"))
        if not isinstance(h0, (int, float)):
            continue
        if math.isclose(float(h0), float(target_h0), rel_tol=0.0, abs_tol=1e-9):
            matches.append(rec)
    ...
    rec = matches[0]
    required = (
        "delta_by_spectrum_signed_fullcov",
        "delta_by_band_signed_fullcov",
        "delta_chi2_act",
    )
    for field in required:
        if field not in rec:
            raise RuntimeError(
                f"Q007_COMPARISON_GATE=FAIL missing_{field} H0={target_h0}"
            )
...
def historical_dominant(rec: dict[str, Any], kind: str) -> str | None:
    ...
    for k in candidates:
        if isinstance(rec.get(k), str):
            return rec[k]
    field = ("delta_by_spectrum_signed_fullcov" if kind == "spectrum" else "delta_by_band_signed_fullcov")
    d = rec.get(field, {})
...
def preflight(c: dict[str, Any], args) -> dict[str, Any]:
    q7base = load_json(args.q007_baseline) if Path(args.q007_baseline).is_file() else {}
    ...
    checks = {
        ...
        "Q007_BASELINE_PARENT_GATE": q7base.get("q") == "Q-007",
    ...
    try:
        q007_baseline_point(c, q7base)
        checks["Q007_BASELINE_POINT_GATE"] = True
    except Exception:
        checks["Q007_BASELINE_POINT_GATE"] = False
...
def run_point(c: dict[str, Any], args) -> dict[str, Any]:
    q7base = load_json(args.q007_baseline)
    ...
    baseline_rec = q007_baseline_point(c, q7base)
    ...
    baseline_vals = extract_param_vector(baseline_rec["fixed_sampled_parameters"], needed - {"H0"})
    ...
    result = {
        ...
        "reoptimized": False,
        "q005_baseline_reconstructed_from_q007_baseline_point": {
            "H0": float(c["historical_baseline"]["h0"]),
            "chi2_scientific_total": float(c["historical_baseline"]["q005_best_known_chi2"]),
            "fixed_sampled_parameters_sha256": canonical_hash(baseline_rec["fixed_sampled_parameters"]),
        ...
        "q007_historical": {
            ...
            "delta_by_spectrum_signed_fullcov": historical.get("delta_by_spectrum_signed_fullcov", {}),
            "delta_by_band_signed_fullcov": historical.get("delta_by_band_signed_fullcov", {}),
