#!/usr/bin/env python3
"""Q016 V10 staged aggregation and primary endpoint certification."""
import argparse,json,math
from pathlib import Path
import yaml

Q="Q016"; RUN="Q016-MATCHED-CMB-SURFACE-V10"; RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-010"
BASIS="SCALAR_PRIMARY_CMB_CHI2_PLUS_NORMALIZATION_FREE_GAUSSIAN_SHAPE_LIKELIHOODS"
BRANCHES=("planck","spt","act")
PARENT_FIXED_RUN="Q016-MATCHED-CMB-SURFACE-V9"
PARENT_FIXED_RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-009"
...
def record_gate(d,branch,mode,expected_run=RUN,expected_result=RESULT):
...
def collect(root,branch,mode,expected_run=RUN,expected_result=RESULT):
    out=[];excluded=[]
    # Artifact downloads preserve internal directories (e.g. q016_v9_phase1/).
    # Recursive discovery is mandatory; V9's non-recursive glob caused the
    # aggregator to report zero records despite all 12 workers succeeding.
    for p in sorted(Path(root).rglob(f"{branch}_{mode}_*.json")):
        ...
        ok,why=record_gate(d,branch,mode,expected_run=expected_run,expected_result=expected_result)
        if ok:out.append(d)
...
def fixed_summary(a,cfg):
    result={"case_id":cfg["project"]["case_id"],"q":Q,"original_case_question":cfg["scientific_question"],
            ...
            "parent_phase1_run_id":PARENT_FIXED_RUN,
            "parent_phase1_result_id":PARENT_FIXED_RESULT,
            "parent_phase1_github_run_id":33749345635,
            "parent_phase1_head_sha":"35fa2038456e8c2e40885b37d5ea27e595eac4b2",
            "recovered_from_parent_artifacts":True,
            "status":"PASS","branches":{},"excluded_records":[]}
    all_complete=True
    for b in BRANCHES:
        recs,ex=collect(a.input_dir,b,"fixed_h0_71p5",
                        expected_run=PARENT_FIXED_RUN,expected_result=PARENT_FIXED_RESULT)
...
def final(a,cfg):
    ...
    if fixed.get("parent_phase1_run_id")!=PARENT_FIXED_RUN or fixed.get("parent_phase1_result_id")!=PARENT_FIXED_RESULT:
        raise RuntimeError("PARENT_PHASE1_PROVENANCE_GATE=FAIL")
    ...
    gates.update({
        ...
        "PARENT_PHASE1_PROVENANCE_GATE":
            fixed.get("parent_phase1_run_id")==PARENT_FIXED_RUN and
            fixed.get("parent_phase1_result_id")==PARENT_FIXED_RESULT and
            int(fixed.get("parent_phase1_github_run_id",-1))==33749345635 and
            fixed.get("parent_phase1_head_sha")=="35fa2038456e8c2e40885b37d5ea27e595eac4b2",
    })
...
def main():
    ...
    ap.add_argument("--config",default="q016_matched_cmb_surface_v10_config.yml")
