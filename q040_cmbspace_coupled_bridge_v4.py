#!/usr/bin/env python3
"""Bubbleverse Q-040 CMB-space coupled bridge V4.

V4 preserves the V3 exact-GLS variable-projection solver unchanged and repairs only frozen Q032 workflow-artifact provenance.
...
The V3 MAP mechanism remains unchanged:

V3/V4: exact-GLS variable projection. The latent identity-multipole CMB vector is
...
INTERVENTION = "NATIVE_NUISANCE_MARGINALIZED_CMB_SPACE_LAPLACE_SCHUR_V4"
V3_FAILED_RUN_ID = 34209404762
...
# metadata and prereg/source-lock checks become V4-identical.
base.Q = Q
...
def variable_projection_map(ctx: NativeContext, eta0: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "converged": converged,
        "solver": "EXACT_GLS_VARIABLE_PROJECTION_PROFILED_NUISANCE_MAP_V4",
        "objective": float(obj),
...
def map_start(args: Any) -> int:
    ...
    rec: dict[str, Any] = {
        ...
        "technical_supersession": f"Q040-CMBSPACE-V3_RUN_{V3_FAILED_RUN_ID}_Q032_ARTIFACT_PROVENANCE_REPAIR",
    }
    ...
    try:
        ...
        if rec.get("status") == "COMPLETE":
            print(f"Q040_V4_MAP_GATE=PASS implementation={args.implementation} start={args.start_family}")
        else:
            print(f"Q040_V4_MAP_GATE=FAIL implementation={args.implementation} start={args.start_family} status={rec.get('status')} stage={rec.get('failure_stage')}", file=sys.stderr)
            if rec.get("error"):
                print(rec.get("traceback", rec["error"]), file=sys.stderr)
    return 0 if rec.get("status") == "COMPLETE" else 2


# Install V4 numerical functions into reused V2 downstream logic. In particular,
# compression validation calls base.optimize_eta, so it must use the V4 scaled
