...
Q="Q-041"; PID="Q041-PLANCKPORT-V16"; RID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-016"
...
def static(a):
    ...
    tr=pre["technical_revision"]; assert tr["scientific_and_numerical_settings_changed_from_v15"] is False
    assert tr["fresh_v16_chain_start_required"] is True and tr["same_v16_series_resume_only"] is True
    assert tr["cross_version_chain_state_reuse_forbidden"] is True and tr["cobaya_allow_changes_used"] is False
    ...
    ent=reg["programs"][PID]; assert ent["status"]=="ACTIVE" and ent["workflow_id"]=="q041-planck-portability-v16.yml"
    assert ent["same_series_resume_only"] is True and ent["cross_version_chain_state_reuse_forbidden"] is True
    assert reg["programs"]["Q041-PLANCKPORT-V15"]["status"]=="BROKEN" and reg["programs"]["Q041-PLANCKPORT-V15"]["superseded_by"]==PID
    for token in ["Q041-PLANCKPORT-V16","R-Q041-EDE-DOWNSTREAM-PORTABILITY-016","series_id:",
                  "q041-state-v16-${{ inputs.series_id }}-","q041_chain_metadata_v16.json",
                  "Require isolated V16 continuation state","Enforce fresh V16 state at segment 1",
                  ...
                  "q032-preflight-sealed-v2","q032-hillipop-covariance-v2","Dispatch next V16 segment",
                  '"series_id":os.environ["SERIES_ID"]']:
        assert token in wf, token
    for forbidden in ["q041-state-v15-${{ matrix.implementation }}","Restore V13 partial chain","_adopt_v14_metadata","allow_changes=True"]:
        assert forbidden not in (wf+program), forbidden
    for token in ["_resume_provenance_gate","V16_SAME_SERIES_RESUME_PROVENANCE_GATE","V16_FRESH_SEGMENT1_STATE_GATE",
    ...
    dump(a.output,{"q":Q,"program_id":PID,"stage":"STATIC_TESTS","status":"PASS","gates":{
      "IDENTITY":"PASS","V15_SCIENCE_LOCK":"PASS","FRESH_SEGMENT1":"PASS","SAME_SERIES_RESUME_ONLY":"PASS",
      "CROSS_VERSION_STATE_FORBIDDEN":"PASS","NO_ALLOW_CHANGES":"PASS","CONTROLLED_NO_SCIENCE_GREEN_FINAL":"PASS",
      "TECHNICAL_FAILURE_RED":"PASS","Q032_V2_FREEZE":"PASS"}})
    print("Q041_V16_STATIC_TEST_GATE=PASS"); return 0
def final(a):
    ...
    print(f"Q041_V16_FINAL_TEST_GATE=PASS classification={cls}"); return 0
def main():
