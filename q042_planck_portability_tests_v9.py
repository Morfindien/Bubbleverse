...
def static(a):
 ...
 gates={
  ...
  "WORKFLOW_IDENTITY":"PROGRAM_ID: Q042-PREFLIGHT-V9" in w and "CURRENT_Q: Q-042" in w,
  ...
  "V7_CACHE_REUSE":"q042-v7-${{ runner.os }}-py311-cobaya356-q032-4dc873a-pc1222-pybobyqa150-pantheonplus" in w,
  ...
  "PILOT_CACHE_FAIL_CLOSED":"q042-v9-${{ runner.os }}-py311-cobaya356-q032-4dc873a-pc1222-pybobyqa150-pantheonplus" in w and "fail-on-cache-miss: true" in w,
  ...
  "WORKFLOW_SELF_REFERENCE":"--workflow .github/workflows/q042-planck-portability-v9.yml" in w,
  "WORKFLOW_REGISTRY_PATCH_SELF_REFERENCE":"--registry-patch q042_program_registry_patch_v9.json" in w and "--registry-patch q042_program_registry_patch_v7.json" not in w and "--registry-patch q042_program_registry_patch_v6.json" not in w,
  "NO_ACTIVE_STALE_V8_REFERENCES":not any(x in p or x in w for x in ("q042_v8_cell_preflight","q042_bobyqa_start{start_index}_v8.json","q042-pilot-*-v8","q042_preflight_tests_v8.json")),
  "ACTIVE_V9_WORKFLOW_OUTPUTS":all(x in w for x in ("q042_static_program_v9.json","q042_static_tests_v9.json","q042_primary_nonoverlap_support_v9.json","q042_hillipop_nonoverlap_precision_v9.npy","q042_runtime_preflight_v9.json","q042_preflight_final_v9.json")),
  "PILOT_RESULT_VERSION":"q042_pilot_result_v9.json" in p and all(x not in p for x in ("q042_pilot_result_v4.json","q042_pilot_result_v5.json","q042_pilot_result_v6.json","q042_pilot_result_v7.json")),
  "REGISTRY_ARTIFACT_VERSION":all(str(x).endswith("v9") or "*-v9" in str(x) for x in entry.get("result_artifacts",[])) and len(entry.get("result_artifacts",[]))==4,
  "REGISTRY_ENTRY":entry.get("q")==Q and entry.get("status")=="ACTIVE" and entry.get("workflow_id")=="q042-planck-portability-v9.yml",
 ...
 print("Q042_V9_STATIC_TEST_GATE=PASS")
def final(a):
 ...
 if status!="PASS":raise SystemExit(2)
 print("Q042_V9_FINAL_TEST_GATE=PASS")
def parser():
