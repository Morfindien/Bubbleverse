#!/usr/bin/env python3
"""Bubbleverse Q041 — downstream scientific-consequence portability V3.
...
Q="Q-041"
PROGRAM_ID="Q041-PLANCKPORT-V3"
RUN_ID="Q041-DOWNSTREAM-SCIENTIFIC-CONSEQUENCE-PORTABILITY-V3"
RESULT_ID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-003"
...
def load_q032(root: str|Path):
    ...
    import q032_planck_tt3pair_bridge_v3 as q32  # type: ignore
...
def q032_cfg(q32: Any, root: str|Path) -> dict[str,Any]:
    c=q32.load_cfg(Path(root)/"q032_planck_tt3pair_bridge_v3_config.yml")
...
def sample(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_json(Path(args.output_dir)/"q041_chain_metadata_v3.json",rec)
...
def aggregate(args: argparse.Namespace) -> int:
    load_preregister(args.preregister); load_source_lock(args.source_lock)
    metas=[]
    for p in Path(args.input_dir).rglob("q041_chain_metadata_v3.json"):
        d=read_json(p)
