#!/usr/bin/env python3
"""Bubbleverse Q-040 defensive native-prior RQMC CMB marginalization V1.
...
Q = "Q-040"
PROGRAM_ID = "Q040-RQMC-V1"
RUN_ID = "Q040-DEFENSIVE-RQMC-CMB-MARGINAL-V1"
RESULT_ID = "R-Q040-EDE-DEFENSIVE-RQMC-CMB-MARGINAL-001"
...
def _seed(*parts: Any) -> int:
    h = hashlib.sha256("|".join(map(str, (PROGRAM_ID,) + parts)).encode()).digest()
...
def prepare(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for impl in IMPLEMENTATIONS:
            ...
            cfile = outdir / f"q040_rqmc_{impl}_centers_v1.npz"
            bfile = outdir / f"q040_rqmc_{impl}_basis_v1.npz"
        ...
        pfile = outdir / "q040_rqmc_probes_v1.npz"
        save_npz(pfile, cmb=cmbs, A_planck=apl, ells=np.asarray(ell_ref, dtype=int))
        pmeta = outdir / "q040_rqmc_probes_v1.json"
...
def rqmc_shard(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        rec.update({"status": "PASS", "shell_start_per_block": start, "shell_end_per_block": end,
                    ...
                    "logsum_wL_by_replicate_probe": logsums.tolist(), "coreset_mass_by_k_replicate": out_mass,
...
def merge_level(args: argparse.Namespace) -> int:
    ...
    for impl in IMPLEMENTATIONS:
        ...
        for d in ss:
            ...
            seen.add(key)
            shell_log=_combine_logsum(shell_log,np.asarray(d["logsum_wL_by_replicate_probe"],dtype=float))
        ...
        if previous:
            ...
            cum_log=_combine_logsum(np.asarray(pp["logsum_wL_by_replicate_probe"],dtype=float), shell_log)
        ...
        state["implementations"][impl]={
            "counts_by_replicate":cum_count.tolist(),"logsum_wL_by_replicate_probe":cum_log.tolist(),
