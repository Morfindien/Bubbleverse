#!/usr/bin/env python3
"""Bubbleverse Q014 V10 — V8 basin-replication numerical certification.

V10 preserves the V8 scientific surface and normalization-free objective exactly.
V8 completed every task but correctly failed MULTISTART_STABILITY_GATE because
several free/fixed profiles converged to distinct optimizer basins, especially the
Planck/NPIPE approximation. V10 does not relax the <=1.0 stability requirement.

For each free/fixed profile V10 launches four deterministic continuations:
  1) the exact V8 best minimum from the same mode,
  2) a small bounded jitter around that minimum,
  3) the exact V8 best minimum from the paired mode (H0 released/projected),
  4) a small bounded jitter around that paired-mode minimum.

The Q011-shared-physics profile remains a single chain-native nuisance profile.
V8 is a numerical continuation parent only and is not promoted to a scientific
Q014 result.
"""
...
RUN_ID="Q014-EXTERNAL-VIABILITY-V10"
CODE_VERSION="8.0-v8-basin-replication-certification"
V8_EXPECTED_RUN="Q014-EXTERNAL-VIABILITY-V8"
V8_EXPECTED_RESULT="R-Q014-EDE-EXTERNAL-VIABILITY-005"
V8_RESULT_PATH: Path|None=None
...
def inject_chain_native_parameters(info:dict[str,Any], chain:str, c:Mapping[str,Any])->None:
    ...
    if chain=="planck_npipe_k039_approx":
        ...
        # default merging so V8 best nuisance values can be deterministic refs.
...
def load_v8_result(path:Path|None=None)->dict[str,Any]:
    p=path or V8_RESULT_PATH
    if p is None: raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL no V8 result path")
    d=json.loads(Path(p).read_text(encoding="utf-8"))
    if d.get("q")!="Q014" or d.get("run_id")!=V8_EXPECTED_RUN or d.get("result_id")!=V8_EXPECTED_RESULT:
        raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL identity")
    if d.get("scientifically_usable_result") is not False or d.get("validation_status")!="NUMERICALLY UNRESOLVED":
        raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL status")
    if d.get("objective_basis")!="SCALAR_TOTAL_LIKELIHOOD_CHI2_PLUS_NORMALIZATION_FREE_GAUSSIAN_CONSTRAINT_SHAPES":
        raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL objective basis")
    jm=d.get("job_manifest",{})
    if int(jm.get("expected",-1))!=15 or int(jm.get("returned",-2))!=15 or jm.get("missing") or jm.get("duplicates") or jm.get("runtime_source_compatible") is not True:
        raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL completeness/provenance")
    # Preserve the actual reason V8 is only a numerical parent.
    primary=["planck_npipe_k039_approx","spt_d1_only"]
    unstable=False
    for chain in primary:
        for mode in ("reference_free_h0","fixed_h0_71p5"):
            pr=d.get("chains",{}).get(chain,{}).get("profiles",{}).get(mode,{})
            unstable |= pr.get("stable") is not True
    if not unstable:
        raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL parent no longer records required instability")
    return d

def v8_best_minimum(chain:str, mode:str)->dict[str,float]:
    d=load_v8_result()
    rec=d["chains"][chain]["profiles"][mode].get("best_record")
    if not isinstance(rec,Mapping) or rec.get("status")!="COMPLETE":
        raise RuntimeError(f"V8_NUMERICAL_PARENT_GATE=FAIL missing best record {chain}/{mode}")
    row=rec.get("minimum")
    if not isinstance(row,Mapping): raise RuntimeError("V8_NUMERICAL_PARENT_GATE=FAIL minimum")
    out={}
...
JITTER_TWEAKS={
    ...
    "Tcal":0.0010, "Ecal":0.0020,
}

def _bounded_jitter(base:dict[str,float], info:Mapping[str,Any], sign:float)->dict[str,float]:
    params=info.get("params",{}) if isinstance(info,Mapping) else {}
    out=dict(base)
    for name,dv in JITTER_TWEAKS.items():
        if name not in out or name not in params: continue
        x=float(out[name])+sign*float(dv)
        spec=params.get(name)
        if isinstance(spec,Mapping):
            prior=spec.get("prior")
            if isinstance(prior,Mapping) and "min" in prior and "max" in prior:
                lo=float(prior["min"]); hi=float(prior["max"]); eps=max((hi-lo)*1e-6,1e-10)
                # If the requested direction hits a boundary, reflect the perturbation inward.
...
def start_vector(chain:str, family:str, c:Mapping[str,Any], info:Mapping[str,Any], q011_vec:Mapping[str,float])->dict[str,float]:
    families={
        "v8_free_best":("reference_free_h0",False,+1.0),
        "v8_free_jitter":("reference_free_h0",True,+1.0),
        "v8_fixed_best":("fixed_h0_71p5",False,-1.0),
        "v8_fixed_jitter":("fixed_h0_71p5",True,-1.0),
    }
    # V10 compatibility repair: V5 smoke/preflight still uses legacy families
    # such as external. Any family not owned by the V8-continuation layer must
    # be delegated unchanged to the frozen V5 implementation.
    if family not in families:
        return _original_start(chain,family,c,info,q011_vec)
    source_mode,jitter,sign=families[family]
    base=impl.q005_default_reference(info)
    base.update(v8_best_minimum(chain,source_mode))
    if source_mode=="fixed_h0_71p5" and "H0" not in base:
        base["H0"]=float(c["scientific_surface"]["target_h0"])
    return _bounded_jitter(base,info,sign) if jitter else base
impl.start_vector=start_vector

LEGACY_START_FAMILIES=("external","q011","baseline","jitter_a","jitter_b")
def legacy_start_family_compatibility_preflight(c:Mapping[str,Any])->dict[str,Any]:
    dummy_info={"params":{}}
    q011_dummy={}
    checks={}
    details={}
    for chain in CONSTRAINTS:
        for family in LEGACY_START_FAMILIES:
            key=f"{chain}:{family}"
            try:
                expected=_original_start(chain,family,c,dummy_info,q011_dummy)
                actual=start_vector(chain,family,c,dummy_info,q011_dummy)
                checks[key]=(actual==expected)
                details[key]={"pass":checks[key],"expected":expected,"actual":actual}
            except Exception as exc:
                checks[key]=False
                details[key]={"pass":False,"error":repr(exc)}
...
def seed_evaluation(chain:str,mode:str,family:str,restart:int,q011_path:str|Path,c:Mapping[str,Any],out:Path)->dict[str,Any]:
    ...
    if not bool(getattr(model.prior,"reference_is_pointlike",False)):
        raise RuntimeError("DETERMINISTIC_V8_CONTINUATION_GATE=FAIL reference not pointlike")
...
def run_task_v9(chain,mode,family,restart,q011_path,v8_path,output,c,max_evals=None)->int:
    global V8_RESULT_PATH
    V8_RESULT_PATH=Path(v8_path)
    load_v8_result()
    ...
    try: seed=seed_evaluation(chain,mode,family,restart,q011_path,c,out)
        ...
        impl.dump_json(out,{"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN_ID,"chain":chain,"mode":mode,
            "start_family":family,"restart_index":restart,"status":"TECHNICAL_FAILURE","failure_type":"V10_SEED_OR_PRIOR_GATE",
            "error":repr(exc),"finite_result":False})
        return 9
    rc=impl.run_task(chain,mode,family,restart,q011_path,out,c,max_evals)
    rec=json.loads(out.read_text())
    rec["v8_numerical_parent"]={"run_id":V8_EXPECTED_RUN,"result_id":V8_EXPECTED_RESULT,
        "source_mode":"reference_free_h0" if family.startswith("v8_free") else ("fixed_h0_71p5" if family.startswith("v8_fixed") else None),
        "jittered":family.endswith("_jitter")}
...
def v8_parent_preflight(path:str|Path)->dict[str,Any]:
    try:
        d=load_v8_result(Path(path)); ok=True; err=None
    except Exception as exc:
        d={}; ok=False; err=repr(exc)
    spreads={}
    for chain,cv in d.get("chains",{}).items():
        spreads[chain]={m:p.get("best_two_delta_chi2") for m,p in cv.get("profiles",{}).items()}
...
def cli()->int:
    ...
    p=sub.add_parser("preflight"); p.add_argument("--q011",required=True); p.add_argument("--v8-result",required=True); p.add_argument("--environment",action="store_true"); p.add_argument("--chain",choices=list(CONSTRAINTS)); p.add_argument("--output",required=True)
    ...
    p=sub.add_parser("run"); p.add_argument("--chain",choices=list(CONSTRAINTS),required=True); p.add_argument("--mode",choices=["reference_free_h0","fixed_h0_71p5","q011_shared_physics"],required=True); p.add_argument("--start-family",choices=["v8_free_best","v8_free_jitter","v8_fixed_best","v8_fixed_jitter","q011"],required=True); p.add_argument("--restart-index",type=int,required=True); p.add_argument("--q011",required=True); p.add_argument("--v8-result",required=True); p.add_argument("--max-evals",type=int); p.add_argument("--output",required=True)
    ...
    if a.cmd=="preflight":
        global V8_RESULT_PATH; V8_RESULT_PATH=Path(a.v8_result)
        r=impl.preflight(c,a.q011,a.environment,a.chain); vp=v8_parent_preflight(a.v8_result); r["checks"]["V8_NUMERICAL_PARENT_GATE"]=vp["pass"]; r["v8_parent"]=vp
        ...
        legacy=legacy_start_family_compatibility_preflight(c)
        r["checks"]["LEGACY_START_FAMILY_COMPATIBILITY_GATE"]=legacy["pass"]
        r["legacy_start_family_compatibility"]=legacy
    ...
    if a.cmd=="run": return run_task_v9(a.chain,a.mode,a.start_family,a.restart_index,a.q011,a.v8_result,a.output,c,a.max_evals)
    return 2
