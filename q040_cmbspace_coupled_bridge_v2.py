#!/usr/bin/env python3
"""Bubbleverse Q-040 — coupled native-nuisance-marginalized CMB-space bridge V2.
...
The compression uses a constrained Laplace/Schur construction:
  ...
  2. bounded fixed-point nuisance MAP with exact GLS CMB solves and Anderson acceleration;
...
Q = "Q-040"
PROGRAM_ID = "Q040-CMBSPACE-V2"
RUN_ID = "Q040-CMBSPACE-COUPLED-BRIDGE-V2"
RESULT_ID = "R-Q040-EDE-CMBSPACE-COUPLED-BRIDGE-002"
INTERVENTION = "NATIVE_NUISANCE_MARGINALIZED_CMB_SPACE_LAPLACE_SCHUR_V2"
...
def _clip_bounds(x: np.ndarray, bounds: Sequence[tuple[float | None, float | None]]) -> np.ndarray:
    y = np.asarray(x, dtype=float).copy()
    for i, (lo, hi) in enumerate(bounds):
        if lo is not None:
            y[i] = max(y[i], float(lo))
        if hi is not None:
            y[i] = min(y[i], float(hi))
    return y


def anderson_candidate(
    x_hist: Sequence[np.ndarray], f_hist: Sequence[np.ndarray], g: np.ndarray,
    bounds: Sequence[tuple[float | None, float | None]], memory: int = 4,
) -> np.ndarray | None:
    """Type-II Anderson candidate for fixed point g(x), with hard-bound clipping.

    f_k = g_k - x_k. This changes only the numerical path to the same fixed
    point; the likelihood, priors, bounds, latent CMB representation and all
    scientific acceptance gates are unchanged.
    """
    if len(x_hist) < 2 or len(f_hist) < 2:
        return None
    m = min(int(memory), len(x_hist) - 1)
    xs = [np.asarray(v, dtype=float) for v in x_hist[-(m + 1):]]
    fs = [np.asarray(v, dtype=float) for v in f_hist[-(m + 1):]]
    dX = np.column_stack([xs[j + 1] - xs[j] for j in range(m)])
    dF = np.column_stack([fs[j + 1] - fs[j] for j in range(m)])
    fk = fs[-1]
    try:
        gamma, *_ = np.linalg.lstsq(dF, fk, rcond=1e-10)
        cand = np.asarray(g, dtype=float) - (dX + dF) @ gamma
    except Exception:
        return None
    if not np.all(np.isfinite(cand)):
        return None
    return _clip_bounds(cand, bounds)


def accelerated_map(ctx: NativeContext, eta0: np.ndarray) -> dict[str, Any]:
    """Solve the same alternating GLS/nuisance fixed point as V1, but accelerate it.

    V1 demonstrated that all six starts had tiny objective drift while the raw
    nuisance fixed-point displacement remained above the preregistered 1e-3
    tolerance at the hard 12-iteration cap. V2 preserves the original 1e-6
    objective and 1e-3 scaled-nuisance convergence gates, but uses bounded
    Anderson acceleration and a preregistered technical cap of 24 iterations.
    """
    ...
    for outer in range(24):
        ...
        eta_raw, opt = optimize_eta(ctx, cmb0, eta, A_planck=1.0)
        eta_raw = _clip_bounds(eta_raw, ctx.bounds)
        cmb_raw, _, _, _, _ = ctx.gls_system(eta_raw, A_planck=1.0)
        obj_raw = ctx.joint_objective(cmb_raw, eta_raw, A_planck=1.0)
        scaled_eta = float(np.max(np.abs((eta_raw - eta) / np.maximum(ctx.proposal_scales, 1e-12)))) if len(eta) else 0.0
        rel_obj = math.inf if prev_obj is None else abs(obj_raw - prev_obj) / max(1.0, abs(prev_obj))
        f = eta_raw - eta
        x_hist.append(eta.copy()); f_hist.append(f.copy())

        acceleration = {"attempted": False, "accepted": False}
        eta_next = eta_raw.copy()
        cand = anderson_candidate(x_hist, f_hist, eta_raw, ctx.bounds, memory=4)
        if cand is not None and outer >= 1:
            acceleration["attempted"] = True
            try:
                cmb_acc, _, _, _, _ = ctx.gls_system(cand, A_planck=1.0)
                obj_acc = ctx.joint_objective(cmb_acc, cand, A_planck=1.0)
                acceleration["candidate_objective"] = float(obj_acc)
                # Acceleration may alter the path but is never allowed to accept
                # a materially worse joint point than the unaccelerated map.
                allowance = max(1e-8, 1e-7 * max(1.0, abs(float(obj_raw))))
                if finite(obj_acc) and float(obj_acc) <= float(obj_raw) + allowance:
                    eta_next = cand
                    acceleration["accepted"] = True
            except Exception as exc:
                acceleration["error"] = repr(exc)
    ...
    if not converged:
        eta = last_raw
        cmb, _, _, _, _ = ctx.gls_system(eta, A_planck=1.0)
    grad = ctx.grad_cmb(cmb, eta)
    grad_rms = float(np.sqrt(np.mean(grad ** 2)))
    return {
        "status": "COMPLETE" if converged else "NOT_CONVERGED",
        "converged": converged,
        "solver": "BOUNDED_ANDERSON_ACCELERATED_ALTERNATING_GLS_NUISANCE_MAP_V2",
...
def map_start(args: argparse.Namespace) -> int:
    ...
    rec: dict[str, Any] = {
        ...
        "technical_supersession": "Q040-CMBSPACE-V1_RUN_34199777865_FIXED_POINT_NONCONVERGENCE",
    }
    ...
    try:
        ...
        stage = "ACCELERATED_MAP"
        out = accelerated_map(ctx, eta0)
        rec.update(out)
        ...
        try:
            ...
            rec.setdefault("close_error", repr(close_exc))
        write_json(args.output, rec)
        if rec.get("status") != "COMPLETE":
            print(f"Q040_V2_MAP_GATE=FAIL implementation={args.implementation} start={args.start_family} status={rec.get('status')} stage={rec.get('failure_stage')}", file=sys.stderr)
            if rec.get("error"):
                print(rec.get("traceback", rec["error"]), file=sys.stderr)
        else:
            print(f"Q040_V2_MAP_GATE=PASS implementation={args.implementation} start={args.start_family}")
...
def build_compression(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        prefix = f"q040_cmbspace_{args.implementation}_v2"
        ...
        rec.update({"status": "COMPRESSION_BUILD_FAIL", "failure_class": "VALIDATION_OR_NUMERICAL", **failure_record(exc, "COMPRESSION_BUILD")}); print(rec["traceback"], file=sys.stderr)
        write_json(args.metadata_output, rec)
        return 2
    finally:
        ctx.close()
...
def profile(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        rec.update({"status": "PARTIAL_SOFT_STOP", "failure_class": "HPC", **failure_record(exc, "Q040_ENDPOINT")}); print(rec["traceback"], file=sys.stderr)
    except Exception as exc:
        rec.update({"status": "FAILED", "failure_class": "NUMERICAL_OR_LIKELIHOOD", **failure_record(exc, "Q040_ENDPOINT")}); print(rec["traceback"], file=sys.stderr)
    finally:
        signal.alarm(0)
        signal.signal(signal.SIGALRM, old)
        try:
...
def main() -> None:
    ...
    def context_args(a: argparse.ArgumentParser) -> None:
        ...
        a.add_argument("--scratch", default="q040_v2_runtime")
