#!/usr/bin/env python3
"""Q016 V15 static/runtime objective tests and final gate validation."""
import argparse,ast,json,math
from pathlib import Path
import yaml

RUN="Q016-MATCHED-CMB-SURFACE-V15"
RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-015"
...
def static_test(cfg_path,output):
    ...
    src=Path("q016_objective_reprofile_v15.py").read_text()
    agg=Path("q016_aggregate_v15.py").read_text()
    wf=Path(".github/workflows/q016-matched-cmb-surface-v15.yml").read_text() if Path(".github/workflows/q016-matched-cmb-surface-v15.yml").exists() else Path("q016-matched-cmb-surface-v15.yml").read_text()
    ...
    checks={
        ...
        "FROZEN_Q005_REQUIREMENTS_GATE":
            ...
            'q005_hpc_v15_requirements.txt' not in wf,
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V15 objective.
    #
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v15_config.yml");p.add_argument("--output",required=True)
