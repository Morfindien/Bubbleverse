...
Q = "Q018"
RUN = "Q018-PLANCK-LIKELIHOOD-BRIDGE-V3"
RESULT = "R-Q018-EDE-PLANCK-LIKELIHOOD-BRIDGE-003"
...
def load_q016_lock(c: Mapping[str,Any]) -> dict:
    ...
    # The authoritative Q016 V16 endpoint lock does NOT carry q/run_id/result_id
    # at the top level. Its provenance is encoded in parent metadata and in the
    # source identities of each certified endpoint. Validate the actual schema
    # instead of imposing a synthetic top-level identity.
    if d.get("case_id") != "NOT DOCUMENTED":
        raise RuntimeError("Q016_ENDPOINT_LOCK_CASE_GATE=FAIL")

    parent=d.get("parent",{})
    if int(parent.get("github_run_id",-1)) != 33785257733:
        raise RuntimeError("Q016_ENDPOINT_LOCK_PARENT_RUN_GATE=FAIL")
    if parent.get("head_sha") != "3a154b44cbef4b2a3d10fdb12435105921bfc962":
        raise RuntimeError("Q016_ENDPOINT_LOCK_PARENT_SHA_GATE=FAIL")
    if parent.get("github_conclusion") != "success":
        raise RuntimeError("Q016_ENDPOINT_LOCK_PARENT_STATUS_GATE=FAIL")

    planck=d.get("endpoints",{}).get("planck",{})
    free=planck.get("reference_free_h0",{})
    fixed=planck.get("fixed_h0_71p5",{})

    if free.get("source_run_id") != Q016_RUN or free.get("source_result_id") != Q016_RESULT:
        raise RuntimeError("Q016_FREE_ENDPOINT_SOURCE_IDENTITY_GATE=FAIL")
    # The fixed endpoint is authentically inherited from the certified Q016 V9
    # phase-1 result and preserved by V16; do not falsely require a V16 source id.
    if fixed.get("source_run_id") != "Q016-MATCHED-CMB-SURFACE-V9":
        raise RuntimeError("Q016_FIXED_ENDPOINT_SOURCE_RUN_GATE=FAIL")
    if fixed.get("source_result_id") != "R-Q016-EDE-MATCHED-CMB-SURFACE-009":
        raise RuntimeError("Q016_FIXED_ENDPOINT_SOURCE_RESULT_GATE=FAIL")

    if abs(float(free["minimum"]["H0"])-67.988328967159)>1e-10:
        raise RuntimeError("FREE_ENDPOINT_GATE=FAIL")
    if abs(float(free.get("objective_chi2"))-4197.77132517269)>1e-9:
        raise RuntimeError("Q016_FREE_OBJECTIVE_REFERENCE_GATE=FAIL")
    if abs(float(fixed.get("objective_chi2"))-4203.595181947479)>1e-9:
        raise RuntimeError("Q016_FIXED_OBJECTIVE_REFERENCE_GATE=FAIL")

    # fixed H0 is fixed by the profile and therefore absent from its minimum row.
    return d

def q017_compatible_cfg(c: Mapping[str,Any]) -> dict:
    """Build the exact config contract consumed by Q017's frozen full-MF constructor."""
    return {
        "project":{
            "q":"Q017",
            "run_id":Q017_RUN,
        ...
        "parent_q016":{
            ...
            "run_id":Q016_RUN,
            "result_id":Q016_RESULT,
            "github_run_id":33785257733,
            "head_sha":"3a154b44cbef4b2a3d10fdb12435105921bfc962",
            "expected_h0":{"free":67.988328967159,"fixed":71.5},
            "primary_profiles_must_not_be_rerun":True,
            "q011_vector_must_not_be_endpoint":True,
        },
        "frozen_surface":{
            "model":"ede_n3",
            "n_scf":3,
            ...
            "target_h0":71.5,
            "full_planck_likelihood":FULL_LIKE,
            "no_local_h0_prior":True,
            "no_cross_chain_chi2_sum":True,
            "no_combined_interexperiment_significance":True,
        },
        "nuisance":{
...
def build_info(c: Mapping[str,Any], lock: Mapping[str,Any], endpoint: str,
    ...
    q17_endpoint=Q017_ENDPOINT_NAME[endpoint]

    # Q017's builder API uses logical endpoint names "free"/"fixed".
    # Q018 keeps lock-file names externally and translates only at this boundary.
    info0, cosmology, starts0, sampled0 = q17.build_full_mf_info(
        qc, lock, q17_endpoint, restart, prefix
    )
    lock_values={}
    if bridge=="shared3_only":
        lock_values={k:float(starts0[k]) for k in FG6}
    elif bridge=="foreground6_only":
        row=q17.endpoint_record(lock,q17_endpoint)["minimum"]
    ...
    if lock_values:
        info, cosmology, starts, sampled = q17.build_full_mf_info(
            qc, lock, q17_endpoint, restart, prefix, locked_values=lock_values
        )
    else:
