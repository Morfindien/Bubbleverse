...
def static(args):
    ...
    gates={
      ...
      'V3_SOLVER_GATE': comp.get('solver')=='EXACT_GLS_VARIABLE_PROJECTION_PROFILED_NUISANCE_MAP_V3' and int(comp.get('profile_max_iterations'))==5000 and float(comp.get('fixed_point_relative_objective_tolerance'))==1e-6 and float(comp.get('fixed_point_scaled_nuisance_tolerance'))==1e-3 and int(comp.get('verification_triggered_profile_polish_max'))==1,
      'V3_TECHNICAL_SUPERSESSION_GATE': pr.get('technical_supersession',{}).get('supersedes_program_id')=='Q040-CMBSPACE-V2' and pr.get('technical_supersession',{}).get('failed_run_id')==34203696897 and pr.get('technical_supersession',{}).get('scientific_model_data_priors_bounds_support_geometry_and_acceptance_thresholds_changed') is False,
