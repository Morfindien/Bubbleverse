#!/usr/bin/env python3
"""Aggregate Q014 V7 continuation jobs using the cross-mode-comparable profile deviance."""
from __future__ import annotations
import argparse,json,math
from pathlib import Path
import yaml
from q014_external_viability_v7 import impl
ROOT=Path(__file__).resolve().parent
CONFIG_NAME="q014_external_viability_v7_config.yml"
RUN="Q014-EXTERNAL-VIABILITY-V7"
...
def aggregate(records,c,depth):
    ...
    result={"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN,"result_id":c["project"]["result_id"],"original_case_question":c["project"]["original_case_question"],
      "program_status":"V7 NORMALIZATION-FREE CONTINUATION RESULTS AGGREGATED","depth":depth,"q011_globality_status_preserved":c["claim_boundary"]["q011_globality_status_preserved"],
    ...
    result["journal_update"]={"case_id":c["project"]["case_id"],"q":"Q014","program":"q014_external_viability_v7.py","workflow":"q014-external-viability-v7.yml","validation_status":result["validation_status"],"what_was_computed":"V7 cross-mode-comparable external n=3 profile deviance with V6 cross-seeded local certification.","scientific_consequence":"Interpret only after mandatory V7 validation passes; report each chain separately.","what_remains_unresolved":[] if result["scientifically_usable_result"] else ["One or more primary V7 continuation profiles remain numerically unstable or fail nesting monotonicity."],"next_required_action":"ROUTE_VALIDATED_Q014_RESULT" if result["scientifically_usable_result"] else "CONTINUE_NUMERICAL_CERTIFICATION"}
    return result
