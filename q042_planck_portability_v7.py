#!/usr/bin/env python3
"""Bubbleverse Q042 V7 — PolyChord + external-start Py-BOBYQA hard preflight.
...
PROGRAM_ID="Q042-PREFLIGHT-V7"
RUN_ID="Q042-POLYCHORD-BOBYQA-PREFLIGHT-V7"
RESULT_ID="R-Q042-POLYCHORD-BOBYQA-PREFLIGHT-007"
...
def static_check(a:argparse.Namespace)->int:
    ...
    write_json(a.output,out);print("Q042_V7_STATIC_GATE=PASS");return 0
...
def prepare_nonoverlap(a:argparse.Namespace)->int:
    ...
    print("Q042_V7_NONOVERLAP_GATE=PASS");return rc
...
def runtime_preflight(a:argparse.Namespace)->int:
    ...
    cell_dir=Path(a.output).with_suffix("").parent/"q042_v5_cell_preflight";cell_dir.mkdir(parents=True,exist_ok=True)
    ...
    write_json(a.output,out);print("Q042_V7_RUNTIME_PREFLIGHT_GATE=PASS");return 0
...
def pilot_cell(a:argparse.Namespace)->int:
    ...
    write_json(outdir/"q042_pilot_result_v7.json",rec);print(f"Q042_V7_PILOT_GATE=PASS arm={a.arm} model={a.model}");return 0
...
def merge_pilots(a:argparse.Namespace)->int:
    ...
    rows=[]
    for p in Path(a.input_dir).rglob("q042_pilot_result_v7.json"):
        d=read_json(p)
    ...
    write_json(a.output,out);print("Q042_V7_PREFLIGHT_FINAL_GATE=PASS");return 0
