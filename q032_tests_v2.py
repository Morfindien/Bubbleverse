#!/usr/bin/env python3
"""Static and deterministic numerical gates for Bubbleverse Q-032 bridge V2."""
...
def static(output: str) -> int:
    files = [
        "q032_planck_tt3pair_bridge_v2.py",
        "q032_tests_v2.py",
    ]
    for name in files:
    ...
    ast.parse(main_text)
    cfg=yaml.safe_load((ROOT/"q032_planck_tt3pair_bridge_v2_config.yml").read_text(encoding="utf-8"))
    src=json.loads((ROOT/"q032_source_lock_v2.json").read_text(encoding="utf-8"))
    proto=json.loads((ROOT/"q032_protocol_lock_v2.json").read_text(encoding="utf-8"))
    ...
    assert sorted(cfg["implementations"]["hillipop"]["removed_nonparticipating_parameters"]) == ["AdustP", "AsyncP", "beta_dustP"]
    ...
    q32.load_cfg(ROOT/"q032_planck_tt3pair_bridge_v2_config.yml")
    ...
    synth_path=ROOT/"q032_static_synthetic_covariance_v2.json"
    ...
    assert "load_hillipop_tt_native_param_names" in main_text
    assert "HILLIPOP_UNUSED_POLARIZATION_PARAMETER_GATE" in main_text
