#!/usr/bin/env python3
"""Bubbleverse Q014 V8 — objective comparability + numerical continuation repair.

V8 preserves the V5/V6 likelihood/model surface but fixes a statistical accounting
...
comparable. V8 reports a normalization-free profile deviance:
...
constraint shapes still participate in nuisance profiling. V8 then performs
...
RUN_ID="Q014-EXTERNAL-VIABILITY-V8"
...
def run_task_v8(chain,mode,family,restart,q011_path,v6_path,output,c,max_evals=None)->int:
    ...
    try: seed=seed_evaluation(chain,mode,family,restart,q011_path,c,out)
        ...
        impl.dump_json(out,{"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN_ID,"chain":chain,"mode":mode,
            "start_family":family,"restart_index":restart,"status":"TECHNICAL_FAILURE","failure_type":"V8_SEED_OR_PRIOR_GATE",
    ...
    if rec.get("status")=="COMPLETE":
        ...
        tol=float(c["execution"]["v8_seed_policy"]["seed_objective_tolerance"])
...
def cli()->int:
    ...
    if a.cmd=="run": return run_task_v8(a.chain,a.mode,a.start_family,a.restart_index,a.q011,a.v6_result,a.output,c,a.max_evals)
    return 2
