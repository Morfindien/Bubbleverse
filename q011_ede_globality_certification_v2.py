#!/usr/bin/env python3
"""
Bubbleverse Q011 — n=3 EDE fixed-H0 globality / basin certification V2.

V2 is a minimal orchestration patch over Q011 V1. It deliberately preserves the
...
DEFAULT_CONFIG = "q011_ede_globality_certification_v2_config.yml"
CODE_VERSION = "2.0-q011-v1-orchestration-repair"
CURRENT_Q = "Q011"

# Ensure V1 aggregate/result helpers record V2 program identity when delegated.
...
def run_optimizer(
    ...
    """
    V2 orchestration-equivalent replacement for V1 run_optimizer.
    ...
    # V2 FIX-002: frozen Q005 V14 interface is cobaya_run(path, cfg).
    base = v1.base_cfg(c)
...
def technical_gate(c: dict[str, Any], q008: dict[str, Any], output: str) -> int:
    ...
    - the repaired two-argument cobaya_run interface executes a minimal optimizer
      job through the same frozen Q005 V14 backend.
    ...
    obj["output"] = str(
        (ROOT / c["paths"]["results"] / "cobaya_q011_v2_technical_gate").resolve()
    )
    rendered = ROOT / c["paths"]["rendered"] / "q011_v2_technical_gate.yaml"
    ...
    # Critical interface smoke: this is exactly the call signature V1 got wrong.
    rc = core.cobaya_run(rendered, base)
    report["checks"]["COBAYA_INTERFACE_EXECUTION_GATE"] = {
        "pass": int(rc) == 0,
        "exit_code": int(rc),
    ...
    mandatory = [
        ...
        report["checks"]["COBAYA_INTERFACE_EXECUTION_GATE"]["pass"],
    ]
...
def main() -> int:
    ...
    if a.cmd == "preflight":
        ...
        result["v2_patch_gate"] = (
            c["project"]["run_id"] == "Q011-EDE-GLOBALITY-CERTIFICATION-V2"
        )
        result["status"] = "PASS" if (
            result["status"] == "PASS" and result["v2_patch_gate"]
        ) else "FAIL"
    ...
    if v1.preflight(c, q8)["status"] != "PASS":
        raise SystemExit("Q011_V2_PREFLIGHT_GATE=FAIL")
