...
def synthetic_anderson_gate(mod):
    # Static numerical sanity only: candidate must remain finite and inside hard bounds.
    xs=[np.array([0.0,0.0]),np.array([0.5,-0.25]),np.array([0.75,-0.375])]
    gs=[np.array([0.5,-0.25]),np.array([0.75,-0.375]),np.array([0.875,-0.4375])]
    fs=[g-x for x,g in zip(xs,gs)]
    c=mod.anderson_candidate(xs,fs,gs[-1],[(-2.0,2.0),(-2.0,2.0)],memory=2)
    return bool(c is not None and np.all(np.isfinite(c)) and np.all(c>=-2.0) and np.all(c<=2.0))
...
def static(args):
    ...
    gates={
      ...
      'V2_SOLVER_GATE': comp.get('solver')=='BOUNDED_ANDERSON_ACCELERATED_ALTERNATING_GLS_NUISANCE_MAP_V2' and int(comp.get('outer_iteration_max'))==24 and int(comp.get('anderson_memory'))==4 and float(comp.get('outer_relative_objective_tolerance'))==1e-6 and float(comp.get('outer_scaled_nuisance_tolerance'))==1e-3,
      'V2_TECHNICAL_SUPERSESSION_GATE': pr.get('technical_supersession',{}).get('supersedes_program_id')=='Q040-CMBSPACE-V1' and pr.get('technical_supersession',{}).get('scientific_model_data_priors_bounds_support_geometry_and_acceptance_thresholds_changed') is False,
      'SYNTHETIC_ANDERSON_GATE': synthetic_anderson_gate(mod),
