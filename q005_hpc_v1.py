...
def preflight(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    for pkg in ("cobaya", "PyYAML", "numpy", "scipy", "mpi4py", "pybobyqa"):
        key = pkg.lower()
        expected = cfg["frozen_software"][key if key != "pyyaml" else "pyyaml"]
        ok, actual = check_version(pkg, expected)
        report["packages"][pkg] = {"expected": expected, "actual": actual, "pass": ok}
    ...
    for mod in ("cobaya", "yaml", "numpy", "scipy", "act_dr6_cmbonly"):
        try:
...
def collect(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "gates": {},
        "provenance": {
            "config_sha256": sha256_file(ROOT / "q005_hpc_v1_config.yml"),
            "script_sha256": sha256_file(ROOT / "q005_hpc_v1.py"),
        }
    }
...
def main() -> int:
    ...
    ap.add_argument("--config", default="q005_hpc_v1_config.yml")
