...
Q='Q-041'; PID='Q041-PLANCKPORT-V8'; RID='R-Q041-EDE-DOWNSTREAM-PORTABILITY-008'
...
def static(a):
    ...
    assert ent['q']==Q and ent['status']=='ACTIVE' and ent['workflow_id']=='q041-planck-portability-v8.yml'
    assert ent['scientific_program']=='q041_planck_portability_v8.py'
    wf=pathlib.Path(a.workflow).read_text(encoding='utf-8')
    for token in [PID,'q041_prepare_frozen_core_v8.sh','prepare-nonoverlap','runtime-preflight','P_A6_L6_D2','P_L6_D2','P_A6_D2','P_A6_L6','aggregate']:
    ...
    setup=pathlib.Path('q041_setup_v8.sh').read_text(encoding='utf-8')
    core=pathlib.Path('q041_prepare_frozen_core_v8.sh').read_text(encoding='utf-8')
    ...
    assert 'cobaya-install act_dr6_lenslike.ACTDR6LensLike -p "$COBAYA_PACKAGES_PATH"' in setup
    assert 'ACT_LENS_DATA_DIR="$COBAYA_PACKAGES_PATH/data/ACT_dr6_likelihood/v1.2"' in setup
    assert "lens=packages/'data/ACT_dr6_likelihood/v1.2'" in setup
    assert '(cd "$ACT_LENS_DIR" && bash get-act-data.sh)' not in setup
    assert 'external/bao_data_v2_6' in setup
    assert 'external/bao_data_v3_6' not in setup
    assert 'name: q041-chain-${{ matrix.implementation }}-${{ matrix.combo }}-c${{ matrix.chain }}-v8' in wf
    assert 'pattern: q041-chain-*-v8' in wf
    ...
    current_runtime_refs = [
        'q041_planck_portability_v8.py',
        'q041_planck_portability_tests_v8.py',
        'q041_planck_portability_preregister_v8.json',
        'q041_planck_portability_source_lock_v8.json',
        'q041_setup_v8.sh',
        'q041_prepare_frozen_core_v8.sh',
        'q041-static-v8',
        'q041-environment-v8',
        'q041-chain-*-v8',
        'q041-final-v8',
    ...
    stale_runtime_patterns = [
        r'q041_planck_portability_tests_v[3-7]\\.py',
        r'q041_planck_portability_preregister_v[3-7]\\.json',
        r'q041_planck_portability_source_lock_v[3-7]\\.json',
        r'q041_planck_portability_v[3-7]\\.py',
        r'q041_setup_v[3-7]\\.sh',
        r'q041_prepare_frozen_core_v[3-7]\\.sh',
        r'q041-(?:static|environment|final)-v[3-7]\\b',
        r'q041-chain-[^\\n]*-v[3-7]\\b',
    ]
