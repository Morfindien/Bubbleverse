...
Q = "Q023"
RUN = "Q023-FULLMF-BASIN-DIRECTIONS-V2"
...
def q022_final(root):
    """
    Select the authoritative Q022 final result by artifact filename, not merely
    by shared Q/run/result identity.

    The q022-final-v2 artifact intentionally contains several JSON documents
    (merged result, mandatory tests, validated final result, assessment). More
    than one can legitimately carry FINAL_RESULT_GATE=PASS, so identity-only
    selection is ambiguous.
    """
    exact=[]
    for p,d in read_jsons(root):
        if p.name != "q022_final_v2.json":
            continue
        if (d.get("q")=="Q022" and d.get("run_id")==PARENT_Q022_RUN
            and d.get("result_id")==PARENT_Q022_RESULT
    ...
    if len(exact)!=1:
        found=[str(p) for p,_ in exact]
        raise RuntimeError(
            "Q022_AUTHORITATIVE_FINAL_FILE_GATE=FAIL "
            f"expected=1 filename=q022_final_v2.json hits={len(exact)} files={found}"
        )
    return exact[0][1]
