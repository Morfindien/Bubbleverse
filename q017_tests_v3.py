...
Q="Q017"
RUN="Q017-PLANCK-DIRECTION-LOCALIZATION-V3"
...
def static(config, source_lock, workflow, output):
    ...
    wrapper=Path("q017_planck_direction_localization_v3.py").read_text(encoding="utf-8")
    parent_src=Path("q017_planck_direction_localization_v1.py").read_text(encoding="utf-8")
    src=wrapper+"\n"+parent_src
    setup=Path("q017_setup_v3.sh").read_text(encoding="utf-8")
    ...
    checks={
      ...
      "V3_IDENTITY_OVERRIDE_GATE":('impl.RUN = "Q017-PLANCK-DIRECTION-LOCALIZATION-V3"' in wrapper and 'impl.RESULT = "R-Q017-EDE-PLANCK-DIRECTION-LOCALIZATION-003"' in wrapper),
      ...
      "SOURCE_LOCK_HEAD_GATE":sl["repository_head_at_design"]=="db687b0a3f71ee3d0c2288dedeb43949c4dc6a0c",
      "WORKFLOW_REQUIREMENTS_PATH_GATE":("q005_hpc_v14_requirements.txt" in wf and "q005_hpc_v24_requirements.txt" not in wf),
      "PREPARE_BOOTSTRAP_CACHE_INDEPENDENCE_GATE":("cache-dependency-path:" not in wf.split("baseline-profiles:",1)[0]),
    }
    blob_checks={}
    ...
    out={"q":Q,"run_id":RUN,"result_id":RESULT,"test_id":"Q017-V3-STATIC","status":status,"checks":checks,"source_blob_checks":blob_checks}
    write(output,out)
...
def final(result, output):
    ...
    out={"q":Q,"run_id":RUN,"result_id":RESULT,"test_id":"Q017-V3-FINAL","status":status,"checks":checks,"final_result_gate":gate}
    write(output,out)
