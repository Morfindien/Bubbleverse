...
def nesting(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    limit = float(cfg["gates"]["nesting"]["max_abs_delta_chi2"])
    ...
    for name, epsilon in models:
        ...
        if rc == 0:
            ...
            if one:
                ...
                if all(k in comps for k in required):
                    record["chi2"] = sum(float(comps[k]) for k in required)
                    record["bestfit"] = parsed
        records[name] = record

    status = "FAIL"
    deltas: dict[str, Any] = {}
    if all(isinstance(records[m].get("chi2"), (int, float)) for m, _ in models):
        base = float(records["lcdm"]["chi2"])
        for m in ("ede_n3", "ede_n2"):
            d = float(records[m]["chi2"]) - base
            deltas[m] = d
        status = "PASS" if all(abs(float(x)) <= limit for x in deltas.values()) else "FAIL"

    out = {
        "q": "Q-005",
        "project": cfg["project"]["id"],
        "test": "NESTING_COMMON_BACKEND",
        "epsilon_fEDE": eps,
        "max_abs_delta_chi2": limit,
        "records": records,
