...
def rendered_yaml_path(cfg: dict[str, Any], model: str, target_h0: float) -> Path:
    """Return the rendered Cobaya YAML path without mutating the .yaml suffix."""
    tag = f"q006_{model}_h0_{target_h0:.1f}".replace(".", "p")
    return ROOT / cfg["paths"]["rendered"] / f"{tag}.yaml"
...
def render(base: dict[str, Any], cfg: dict[str, Any], model: str, target_h0: float,
    ...
    p = rendered_yaml_path(cfg, model, target_h0)
    p.parent.mkdir(parents=True, exist_ok=True)

    expected_name = f"q006_{model}_h0_{target_h0:.1f}".replace(".", "p") + ".yaml"
    if p.suffix != ".yaml" or p.name != expected_name:
        raise RuntimeError(
            f"RENDERED_YAML_GATE=FAIL path={p} expected_name={expected_name}"
        )

    p.write_text(yaml.safe_dump(obj, sort_keys=False), encoding="utf-8")
    if not p.is_file():
        raise RuntimeError(f"RENDERED_YAML_GATE=FAIL file_not_created path={p}")
...
def preflight(cfg: dict[str, Any]) -> int:
    ...
    probe = rendered_yaml_path(cfg, "ede_n3", 70.853292)
    expected_probe_name = "q006_ede_n3_h0_70p9.yaml"
    ...
    checks = {
        ...
        "rendered_yaml_suffix": probe.suffix == ".yaml",
        "rendered_yaml_name": probe.name == expected_probe_name,
        "legacy_pyaml_bug_absent": not probe.name.endswith("pyaml"),
    }
    d = {
        "q": "Q-006",
        "run_id": cfg["project"]["run_id"],
        "checks": checks,
        "rendered_yaml_probe": str(probe),
        "expected_probe_name": expected_probe_name,
