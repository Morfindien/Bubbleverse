#!/usr/bin/env python3
"""Q014 V9 static and mandatory result validation."""
...
def static():
    c=yaml.safe_load((ROOT/'q014_external_viability_v9_config.yml').read_text()); p=(ROOT/'q014_external_viability_v9.py').read_text(); w=((ROOT/'.github/workflows/q014-external-viability-v9.yml') if (ROOT/'.github/workflows/q014-external-viability-v9.yml').exists() else (ROOT/'q014-external-viability-v9.yml')).read_text(); s=(ROOT/'q014_setup_v9.sh').read_text(); cons=c['scientific_surface']['normalization_free_constraints']
    ...
    gates={
      ...
      'STABILITY_THRESHOLD_PRESERVATION_GATE':c['validation']['max_best_two_delta_chi2']==1.0 and c['execution']['v9_seed_policy']['multistart_threshold_unchanged']==1.0,
      ...
      'MUSE_FROZEN_REUSE_GATE':MUSE in w and 'q014-v9-spt-muse-source' in w and 'q014_setup_v8.sh' in s,
...
def result(path):
    c=yaml.safe_load((ROOT/'q014_external_viability_v9_config.yml').read_text()); r=json.loads(Path(path).read_text()); g={}
    g['Q_IDENTITY_GATE']=r.get('q')=='Q014' and r.get('run_id')==RUN and r.get('result_id')=='R-Q014-EDE-EXTERNAL-VIABILITY-006'
