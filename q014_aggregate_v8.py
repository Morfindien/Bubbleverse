#!/usr/bin/env python3
"""Aggregate Q014 V8 continuation jobs using the cross-mode-comparable profile deviance."""
from __future__ import annotations
import argparse,json,math
from pathlib import Path
import yaml
from q014_external_viability_v8 import impl
ROOT=Path(__file__).resolve().parent
CONFIG_NAME="q014_external_viability_v8_config.yml"
...
def aggregate(records,c,depth):
    ...
    result={"case_id":c["project"]["case_id"],"q":"Q014","run_id":RUN,"result_id":c["project"]["result_id"],"original_case_question":c["project"]["original_case_question"],
      "program_status":"V8 NORMALIZATION-FREE CONTINUATION RESULTS AGGREGATED","depth":depth,"q011_globality_status_preserved":c["claim_boundary"]["q011_globality_status_preserved"],
      ...
      "v6_numerical_parent":c["parent"]["v6"],"job_manifest":{"expected":len(ek),"returned":len(rk),"missing":missing,"duplicates":dupes,"runtime_source_provenance_hashes_by_chain":source,"runtime_source_compatible":source_ok},
    ...
    result["claim_boundary"]={"planck_branch_is_k039_approximation":True,"spt_only_is_primary_independentish_test":True,"spt_plus_desi_is_secondary_and_partially_overlapping":True,"no_cross_chain_absolute_chi2_interpretation":True,"q011_is_not_promoted_to_global":True,"v6_not_interpreted_as_scientific_result":True,"flat_prior_normalization_constants_excluded_from_cross_mode_delta":True}
    result["journal_update"]={"case_id":c["project"]["case_id"],"q":"Q014","program":"q014_external_viability_v8.py","workflow":"q014-external-viability-v8.yml","validation_status":result["validation_status"],"what_was_computed":"V8 cross-mode-comparable external n=3 profile deviance with V6 cross-seeded local certification.","scientific_consequence":"Interpret only after mandatory V8 validation passes; report each chain separately.","what_remains_unresolved":[] if result["scientifically_usable_result"] else ["One or more primary V8 continuation profiles remain numerically unstable or fail nesting monotonicity."],"next_required_action":"ROUTE_VALIDATED_Q014_RESULT" if result["scientifically_usable_result"] else "CONTINUE_NUMERICAL_CERTIFICATION"}
    return result
