...
mkdir -p external q014_v4_source_runtime "$COBAYA_PACKAGES_PATH"
...
# V4: GitHub Actions executes later workflow steps in fresh shells. A shell-local
...
PY
  ...
  printf '%s  %s\n' "$MUSE_SHA" "$MUSE_ZIP" | tee q014_v4_source_runtime/muse_3g_like_march_2025.sha256
...
if chain.startswith('spt_') or chain=='all':
    ...
    out['muse_sha256']=pathlib.Path('q014_v4_source_runtime/muse_3g_like_march_2025.sha256').read_text().split()[0]
pathlib.Path('q014_v4_source_runtime/setup_imports.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
