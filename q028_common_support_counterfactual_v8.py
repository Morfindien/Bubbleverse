#!/usr/bin/env python3
"""
Bubbleverse Q028 V8 — split/checkpointed Criterion B+ continuation.

V8 supersedes V7 as an execution architecture, not as a scientific result.
...
a scientific result was written. V6 preserves the V5 split architecture and removes the monolithic memory/runtime
shape.
...
Q = "Q028"
V6_CONFIG_SCHEMA_LINEAGE = "V5_FAILED_ON_Q017_ELL_BAND_SCHEMA; V6_USES_NAMED_MIN_MAX_BANDS"
V7_FILENAME_CONTRACT_LINEAGE = "V6_SUPPORT_PHASE_EXPECTED_V5_CHECKPOINT_FILENAMES; V7_USES_SELF_CONSISTENT_V7_FILENAMES"
V8_VALIDATION_CONTRACT_LINEAGE = "V7_REACHED_FINAL_CLASSIFICATION_BUT_MISSED_V2_FACTORIAL_ZERO_TOLERANCE; V8_INHERITS_COMPLETE_V2_VALIDATION_CONTRACT"
...
def load_cfg(path: str | Path) -> dict[str, Any]:
    ...
    if not bands or not all(
        ...
        and float(b["min"]) <= float(b["max"])
        for b in bands
    ):
        raise RuntimeError("Q017_ELL_BAND_SCHEMA_GATE=FAIL")

    v = c["validation"]
    frozen_v2_validation = {
        "q017_chi2_closure_abs_tol": 2.0e-4,
        "precision_symmetry_rel_tol": 1.0e-10,
        "shapley_closure_abs_tol": 1.0e-8,
        "factorial_zero_abs_tol": 1.0e-10,
        "minimum_common_support_fraction": 0.9,
        "precision_edge_relative_frobenius_tol": 1.0e-10,
        "precision_edge_relative_max_tol": 1.0e-10,
        "precision_edge_relative_spectral_tol": 1.0e-10,
        "precision_edge_state_quadratic_abs_tol": 2.0e-4,
        "precision_edge_pair_interaction_abs_tol": 2.0e-4,
    }
    for key, expected in frozen_v2_validation.items():
        if key not in v or float(v[key]) != float(expected):
            raise RuntimeError(
                f"V2_VALIDATION_CONTRACT_GATE=FAIL key={key} "
                f"observed={v.get(key)!r} expected={expected!r}"
            )
    return c
...
def precision_filename(mask: int, ek: str) -> str:
    return f"q028_v8_precision_m{mask}_{ek}.json"
...
def parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--config", default="q028_common_support_counterfactual_v8_config.yml"
    )
