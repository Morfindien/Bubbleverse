#!/usr/bin/env python3
"""
Bubbleverse Q-032 — TT-3PAIR common-support Planck likelihood portability bridge V1.

CURRENT_Q: Q-032
RUN_ID: Q032-PLANCK-TT3PAIR-COMMON-SUPPORT-BRIDGE-V1
RESULT_ID: R-Q032-EDE-PLANCK-TT3PAIR-BRIDGE-001
...
CONFIG_FILE = "q032_planck_tt3pair_bridge_v1_config.yml"
SOURCE_LOCK_FILE = "q032_source_lock_v1.json"
PROTOCOL_LOCK_FILE = "q032_protocol_lock_v1.json"
...
def build_hillipop_info(
    ...
    fixed = c["implementations"]["hillipop"]["fixed_nonparticipating_parameters"]
    for name, value in fixed.items():
        if name not in info["params"]:
            raise RuntimeError(f"HILLIPOP_NONPARTICIPATING_PARAMETER_GATE=FAIL missing={name}")
        info["params"][name] = float(value)
        info["likelihood"].pop(f"q031_shape_{name}", None)
    return info
...
def q032_hillipop_runtime_provenance() -> dict[str, Any]:
    p = ROOT / "q032_runtime" / "q032_hillipop_tt_runtime_v1.json"
    if not p.exists():
