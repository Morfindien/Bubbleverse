...
case "$BRANCH" in
  planck|spt|act) ;;
  *) echo "Q016_MECHANISM_V4_SETUP_BRANCH_GATE=FAIL $BRANCH" >&2; exit 2 ;;
esac
...
for p,v in expected.items():
    a=m.version(p)
    assert a==v,(p,a,v)
print("Q016_MECHANISM_V4_PRE_V16_FROZEN_DEPENDENCY_GATE=PASS")
PY
...
if [[ "$BRANCH" == "act" && -f "$ROOT/q016_act_data_v1/data/ACTDR6CMBonly/v1.0/dr6_data_cmbonly.fits" ]]; then
  ...
  echo "Q016_MECHANISM_V4_V1_ACT_ASSET_REUSE_GATE=PASS"
fi
...
for p,v in expected.items():
    a=m.version(p)
    assert a==v,(p,a,v)
print("Q016_MECHANISM_V4_POST_SETUP_FROZEN_STACK_GATE=PASS")
PY
