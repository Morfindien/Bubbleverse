...
def nesting(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    deltas: dict[str, Any] = {}
    if all(
        isinstance(records[m].get("chi2"), (int, float))
    ...
    out = {
        "q": "Q-005",
        "project": cfg["project"]["id"],
        "test": "NESTING_COMMON_BACKEND_FINITE_DOMAIN",
        "fixed_likelihood_nuisance_reference": nuisance_reference,
        "mode": cfg["gates"]["nesting"].get("mode", "BACKEND_FINITE_DOMAIN_CONTINUITY"),
        "ede_production_floor": float(cfg["common_ede_priors"]["fEDE"]["prior"]["min"]),
        "epsilon_fEDE": eps,
        "epsilon_interpretation": "finite EDE-domain continuity point, not mathematical fEDE=0",
