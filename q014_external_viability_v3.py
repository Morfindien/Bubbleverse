#!/usr/bin/env python3
"""Bubbleverse Q014 — n=3 EDE external likelihood viability V3.
...
CURRENT_Q = "Q014"
RUN_ID = "Q014-EXTERNAL-VIABILITY-V3"
CODE_VERSION = "3.0-candl-distribution-chain-native-preflight"
...
def run_task(chain: str, mode: str, start_family: str, restart_index: int,
    ...
    rsp = ROOT / "q014_v3_source_runtime" / "setup_imports.json"
    if rsp.exists():
        try:
...
def preflight(c: Mapping[str, Any], q011_path: str | Path | None = None,
    ...
    if require_environment:
        if chain not in {"planck_npipe_k039_approx", "spt_d1_only", "spt_d1_plus_desi"}:
            ...
            # V3: environment requirements are chain-native. Planck jobs must not
            ...
            for mod in required_modules:
                try:
                    m = __import__(mod)
                    module_checks[mod] = {"pass": True, "file": getattr(m, "__file__", None)}
                except Exception as exc:
