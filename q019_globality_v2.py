...
Q="Q019"
RUN="Q019-PLANCK-COSMOLOGY-GLOBALITY-V2"
RESULT="R-Q019-EDE-PLANCK-COSMOLOGY-REPROFILE-002"
...
def q016_endpoint_check(c,output):
    ...
    for mode in ("reference_free_h0","fixed_h0_71p5"):
        prefix=ROOT/f"q019_v2_q016_eval_{mode}"
        ...
        row=q16.row_from_model_point(model,point)
...
def aggregate(c,input_dir,endpoint_json,output):
    ...
    classification=("LIKELIHOOD DEPENDENCE PRIMARILY HIGH-H0 / LOCAL-GEOMETRY" if final else "INDETERMINATE")
    out={
      "q":Q,"run_id":RUN,"result_id":RESULT,"stage":"V2_FINAL",
