#!/usr/bin/env python3
"""Bubbleverse Q042 V9 — PolyChord + external-start Py-BOBYQA hard preflight.
...
PROGRAM_ID="Q042-PREFLIGHT-V9"
RUN_ID="Q042-POLYCHORD-BOBYQA-PREFLIGHT-V9"
RESULT_ID="R-Q042-POLYCHORD-BOBYQA-PREFLIGHT-009"
...
def static_check(a:argparse.Namespace)->int:
    ...
    write_json(a.output,out);print("Q042_V9_STATIC_GATE=PASS");return 0
...
def prepare_nonoverlap(a:argparse.Namespace)->int:
    ...
    print("Q042_V9_NONOVERLAP_GATE=PASS");return rc
...
def runtime_preflight(a:argparse.Namespace)->int:
    ...
    cell_dir=Path(a.output).with_suffix("").parent/"q042_v9_cell_preflight";cell_dir.mkdir(parents=True,exist_ok=True)
    ...
    write_json(a.output,out);print("Q042_V9_RUNTIME_PREFLIGHT_GATE=PASS");return 0
...
def polychord_stage(a:argparse.Namespace)->int:
    ...
    try:
        ...
        print(f"Q042_V9_POLYCHORD_PROCESS_GATE=PASS action={a.action} arm={a.arm} model={a.model}")
        return 0
    finally:
        try:
...
def bobyqa_stage(a:argparse.Namespace)->int:
    ...
    try:
        ...
        rr.update({"q":Q,"program_id":PROGRAM_ID,"stage":"BOBYQA_PROCESS_STAGE","status":"PASS","start_index":a.start_index,"start_values":starts,"technical_ok":bool(rr["flag"] is not None and int(rr["flag"])>=0 and rr["objective"] is not None),"fresh_process":True,"scientific_classification":"NOT_AVAILABLE"})
    except Exception as exc:
        rr={"q":Q,"program_id":PROGRAM_ID,"stage":"BOBYQA_PROCESS_STAGE","status":"RECORDED_FAILURE","start_index":a.start_index,"start_values":starts,"technical_ok":False,"error":repr(exc),"flag":None,"objective":None,"fresh_process":True,"scientific_classification":"NOT_AVAILABLE"}
    finally:
        try:
            if sm is not None and hasattr(sm,"close"):sm.close()
        except Exception:pass
    write_json(a.result_json,rr)
    print(f"Q042_V9_BOBYQA_PROCESS_GATE={'PASS' if rr['technical_ok'] else 'RECORDED_FAIL'} start={a.start_index} arm={a.arm} model={a.model}")
    return 0
...
def pilot_cell(a:argparse.Namespace)->int:
    ...
    fresh_json=outdir/"q042_polychord_fresh_v9.json"
    resume_json=outdir/"q042_polychord_resume_v9.json"
    ...
    for start_index in range(int(spec["bobyqa"]["pilot"]["external_starts_per_full_cell"])):
        bj=outdir/f"q042_bobyqa_start{start_index}_v9.json"
    ...
    write_json(outdir/"q042_pilot_result_v9.json",rec);print(f"Q042_V9_PILOT_GATE=PASS arm={a.arm} model={a.model}");return 0
...
def merge_pilots(a:argparse.Namespace)->int:
    ...
    rows=[]
    for p in Path(a.input_dir).rglob("q042_pilot_result_v9.json"):
        d=read_json(p)
    ...
    write_json(a.output,out);print("Q042_V9_PREFLIGHT_FINAL_GATE=PASS");return 0
