...
mkdir -p external "$COBAYA_PACKAGES_PATH" q016_results q016_v4_source_runtime
...
d={
 "q":"Q016","run_id":"Q016-MATCHED-CMB-SURFACE-V4",
 "result_id":"R-Q016-EDE-MATCHED-CMB-SURFACE-004",
...
if [[ "$BRANCH" == "act" ]]; then
  ...
  retry_cobaya_install act_dr6_cmbonly.ACTDR6CMBonly
  python - <<'PY'
...
p=pathlib.Path("q016_v4_source_runtime")/f"setup_{b}.json"
p.write_text(json.dumps(d,indent=2,sort_keys=True)+"\n")
