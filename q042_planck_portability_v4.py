#!/usr/bin/env python3
"""Bubbleverse Q042 V4 — PolyChord + external-start Py-BOBYQA hard preflight.
...
PROGRAM_ID="Q042-PREFLIGHT-V4"
RUN_ID="Q042-POLYCHORD-BOBYQA-PREFLIGHT-V4"
RESULT_ID="R-Q042-POLYCHORD-BOBYQA-PREFLIGHT-004"
...
def static_check(a:argparse.Namespace)->int:
    ...
    write_json(a.output,out);print("Q042_V4_STATIC_GATE=PASS");return 0
...
def prepare_nonoverlap(a:argparse.Namespace)->int:
    ...
    print("Q042_V4_NONOVERLAP_GATE=PASS");return rc
...
def runtime_preflight(a:argparse.Namespace)->int:
    ...
    cell_dir=Path(a.output).with_suffix("").parent/"q042_v4_cell_preflight";cell_dir.mkdir(parents=True,exist_ok=True)
    ...
    write_json(a.output,out);print("Q042_V4_RUNTIME_PREFLIGHT_GATE=PASS");return 0
...
def pilot_cell(a:argparse.Namespace)->int:
    ...
    write_json(outdir/"q042_pilot_result_v4.json",rec);print(f"Q042_V4_PILOT_GATE=PASS arm={a.arm} model={a.model}");return 0
...
def merge_pilots(a:argparse.Namespace)->int:
    ...
    rows=[]
    for p in Path(a.input_dir).rglob("q042_pilot_result_v4.json"):
        d=read_json(p)
    ...
    write_json(a.output,out);print("Q042_V4_PREFLIGHT_FINAL_GATE=PASS");return 0
