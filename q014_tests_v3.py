#!/usr/bin/env python3
"""Static and scientific-result gates for Bubbleverse Q014 V3."""
...
def static(config:str,source_lock:str)->dict[str,Any]:
    ...
    setup=(ROOT/"q014_setup_v3.sh").read_text(encoding="utf-8")
...
def result_tests(config:str,result_path:str)->dict[str,Any]:
    ...
    gates["Q_IDENTITY_GATE"]=r.get("q")=="Q014" and r.get("run_id")=="Q014-EXTERNAL-VIABILITY-V3"
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("--config",default="q014_external_viability_v3_config.yml")
    sub=ap.add_subparsers(dest="cmd",required=True)
    p=sub.add_parser("static"); p.add_argument("--source-lock",default="q014_external_viability_v3_source_lock.json"); p.add_argument("--output",required=True)
