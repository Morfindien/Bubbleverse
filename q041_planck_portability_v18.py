#!/usr/bin/env python3
"""Bubbleverse Q041 V18 — single-run linear eight-stage segmented execution.

V18 preserves the Q041 scientific model, likelihoods, priors, sampler thresholds,
classification rules and eight-compute-segment per-chain budget. It replaces the
cross-workflow self-dispatch continuation architecture with one workflow run containing
eight fixed matrix stages in a strict line. Segment 1 starts fresh. Segments 2-8 consume
only the immediately preceding V18 artifact for the same logical chain. HiLLiPoP resume
uses the exact resolved planck_2020_hillipop.TT component serialized in chain.updated.yaml.
No --allow-changes bypass is used.
"""
...
Q="Q-041"
PROGRAM_ID="Q041-PLANCKPORT-V18"
RUN_ID="Q041-DOWNSTREAM-SCIENTIFIC-CONSEQUENCE-PORTABILITY-V18"
RESULT_ID="R-Q041-EDE-DOWNSTREAM-PORTABILITY-018"
V13_PROGRAM_ID="Q041-PLANCKPORT-V13"
V13_PARENT_RUN_ID=34379785115
CHAIN_META="q041_chain_metadata_v18.json"
...
def _lock_hillipop_resume_info(info: dict[str,Any], output_dir: str|Path) -> str:
    ...
    if HILLIPOP_COMPONENT not in likes:
        raise RuntimeError("V18_HILLIPOP_RESUME_COMPONENT_GATE=FAIL component_not_in_new_info")
    likes[HILLIPOP_COMPONENT]=frozen
    if _canonical_hash(likes[HILLIPOP_COMPONENT])!=h:
        raise RuntimeError("V18_HILLIPOP_RESUME_COMPONENT_GATE=FAIL hash_roundtrip")
    print(f"Q041_V18_HILLIPOP_RESUME_LOCK_GATE=PASS hash={h}")
    return h
...
def _resume_provenance_gate(output_dir: str|Path, implementation: str, combo: str, chain: int, series_id: str, segment: int) -> dict[str,Any]:
    """Require the immediately preceding V18 stage for the same logical chain."""
    outdir=Path(output_dir)
    if int(segment) <= 1:
        raise RuntimeError("V18_SEGMENT1_FRESH_START_GATE=FAIL")
    if not _chain_resume_ready(outdir):
        raise RuntimeError("V18_CONTINUATION_STATE_GATE=FAIL missing_resume_files")
    meta_path=outdir/CHAIN_META
    if not meta_path.exists():
        raise RuntimeError("V18_CONTINUATION_STATE_GATE=FAIL missing_metadata")
    try:
        d=base.read_json(meta_path)
    except Exception as exc:
        raise RuntimeError(f"V18_RESUME_METADATA_PARSE_GATE=FAIL {exc!r}")
    if not (
        d.get("q")==Q and d.get("program_id")==PROGRAM_ID
        and d.get("implementation")==implementation and d.get("combo")==combo
        and int(d.get("chain",-1))==int(chain)
        ...
        raise RuntimeError("V18_LINEAR_PREDECESSOR_PROVENANCE_GATE=FAIL")
    print(
        f"Q041_V18_LINEAR_PREDECESSOR_GATE=PASS series={series_id} "
...
def sample(args: argparse.Namespace) -> int:
    ...
    if not str(args.series_id).strip():
        raise RuntimeError("V18_SERIES_ID_GATE=FAIL")

    outdir=Path(args.output_dir)
    outdir.mkdir(parents=True,exist_ok=True)
    prior=None
    if int(args.segment)==1:
        # One workflow, one lineage: segment 1 is intentionally fresh. No V13-V18 chain state is admissible.
        for name in ("chain.1.txt","chain.checkpoint","chain.covmat","chain.updated.yaml",CHAIN_META):
            if (outdir/name).exists():
                raise RuntimeError(f"V18_FRESH_SEGMENT1_GATE=FAIL unexpected={name}")
        compute_segments_used=0
        resuming=False
        print("Q041_V18_FRESH_SEGMENT1_GATE=PASS")
    else:
    ...
    if prior is not None and prior.get("status") in ("COMPLETE","NO_SCIENTIFIC_RESULT"):
        ...
        print(f"Q041_V18_TERMINAL_CHAIN_CARRY_GATE=PASS arm={args.implementation} combo={args.combo} chain={args.chain} status={terminal.get('status')}")
        return 0

    if compute_segments_used >= int(args.max_segments):
        raise RuntimeError("V18_PER_CHAIN_SEGMENT_BUDGET_DISPATCH_GATE=FAIL")
    ...
    if rec["status"]=="PARTIAL_SOFT_STOP":
        ...
        if int(rec["compute_segments_used"])>int(args.max_segments): raise RuntimeError("V18_PER_CHAIN_SEGMENT_BUDGET_GATE=FAIL")
...
def finalize_linear(args: argparse.Namespace) -> int:
    ...
    for p in Path(args.input_dir).rglob(CHAIN_META):
        ...
        if key in rows: raise RuntimeError(f"V18_FINAL_DUPLICATE_CHAIN_META_GATE=FAIL key={key}")
        rows[key]=(d,p)
    expected={(a,c,k) for a in ARMS for c in ALL_COMBOS for k in (0,1)}
    missing=sorted(expected-set(rows))
    if missing: raise RuntimeError(f"V18_FINAL_CHAIN_METADATA_GATE=FAIL missing={missing}")
    known={"COMPLETE","PARTIAL_SOFT_STOP","NO_SCIENTIFIC_RESULT"}
    bad=sorted(k for k,(d,_) in rows.items() if d.get("status") not in known)
    if bad: raise RuntimeError(f"V18_FINAL_TECHNICAL_STATUS_GATE=FAIL bad={bad}")
    wrong_stage=sorted(k for k,(d,_) in rows.items() if int(d.get("segment",0) or 0)!=8)
    if wrong_stage: raise RuntimeError(f"V18_FINAL_SEGMENT8_PROVENANCE_GATE=FAIL bad={wrong_stage}")
    ...
    if no_science:
        ...
        if reasons!={"MAX_SAMPLES_WITHOUT_CONVERGENCE"}:
            raise RuntimeError(f"V18_FINAL_CONTROLLED_REASON_GATE=FAIL reasons={sorted(map(str,reasons))}")
    ...
    if partial:
        ...
        if under: raise RuntimeError(f"V18_FINAL_SEGMENT_BUDGET_ACCOUNTING_GATE=FAIL under={under}")
