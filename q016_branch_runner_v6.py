#!/usr/bin/env python3
"""Bubbleverse Q016 V6 targeted basin-replication branch runner.
...
CURRENT_Q = "Q016"
RUN_ID = "Q016-MATCHED-CMB-SURFACE-V6"
...
def _set_point_ref(params,name,value):
    spec=params.get(name)
    if not isinstance(spec,dict) or "prior" not in spec:
        return False
    prior=spec.get("prior")
...
def apply_targeted_basin_reference(info,branch,mode,restart,cfg):
    restart=int(restart)
    if restart not in (6,7,8):
        raise RuntimeError(f"TARGETED_RESTART_ID_GATE=FAIL restart={restart}")
    lock=json.loads((ROOT/cfg["execution"]["target_basin_lock_file"]).read_text())
    key=f"{branch}|{mode}"
    if key not in lock["targeted_profiles"]:
        raise RuntimeError(f"TARGET_PROFILE_GATE=FAIL profile={key}")
    center=dict(lock["targeted_profiles"][key]["values"])
    dv=lock["restart_perturbations"][str(restart)] if str(restart) in lock["restart_perturbations"] else lock["restart_perturbations"][restart]
    params=info.setdefault("params",{})
    applied={}
    for name,x0 in center.items():
        x=float(x0)+float(dv.get(name,0.0))
        if name=="H0" and mode=="fixed_h0_71p5":
            continue
        if _set_point_ref(params,name,x):
            applied[name]=x
    # Fixed H0 is part of the frozen surface, not an optimizer start.
    if mode=="fixed_h0_71p5":
        assert float(params["H0"])==float(cfg["model"]["target_h0"])
    source=lock["targeted_profiles"][key]["source"]
    return {
      "restart_index":restart,
      "reference_family":cfg["execution"]["restart_families"][restart],
      "target_profile":key,
      "target_center_source":source,
      "target_center_values":center,
      "applied_point_reference_values":applied,
      "pointlike_targeted_start":True,
    }
...
def apply_multistart_reference(info,branch,restart,cfg):
    ...
    variants=[
      ...
      {"H0":+0.25,"fEDE":+0.015,"log10z_c":-0.18,"thetai_scf":+0.35,
       "omega_b":-0.00006,"omega_cdm":+0.0010,"n_s":+0.002},
      {"H0":-0.35,"fEDE":+0.045,"log10z_c":+0.16,"thetai_scf":+0.12,
       "omega_b":+0.00005,"omega_cdm":-0.0010,"n_s":-0.002},
    ]
    idx=int(restart)
    if idx < 0 or idx >= len(variants):
        raise RuntimeError(f"RESTART_ID_GATE=FAIL restart={restart}")
    base=dict(bases[branch]); tweak=variants[idx]
    for k,dv in tweak.items():
        base[k]=float(base.get(k,0.0))+float(dv)
    for k,v in base.items():
        _set_ref(params,k,v)
    return {"restart_index":int(restart),"reference_family":cfg["execution"]["restart_families"][int(restart)],
...
def run(args):
    ...
    status={"q":CURRENT_Q,"run_id":RUN_ID,"result_id":RESULT_ID,"branch":args.branch,
            ...
            "continuation_parent":{"v4_github_run_id":33693382859,"v4_run_id":"Q016-MATCHED-CMB-SURFACE-V4","v4_head_sha":"596ee68677784deb4c28d1eb5992faf6dcd2c5b1","v5_github_run_id":33720410690,"v5_run_id":"Q016-MATCHED-CMB-SURFACE-V5","v5_head_sha":"5148b8b3badbbe313d30e6f744d51d6fd42b584e"}}
    ...
    restart_reference=apply_targeted_basin_reference(info,args.branch,args.mode,args.restart,cfg)
    # Build once before minimisation: imports, data, covariance and theory must all initialise.
    from cobaya.model import get_model
    model=get_model(info)
    # Distribution-based refs are intentional in Q016 multistart profiling.
    # Cobaya reference_is_pointlike=False therefore is diagnostic, not failure.
    import numpy as np
    reference_is_pointlike=bool(model.prior.reference_is_pointlike)
    if not reference_is_pointlike:
        raise RuntimeError("TARGETED_POINTLIKE_REFERENCE_GATE=FAIL")
    reference_seed=1601600 + 100*int(args.restart) + {"planck":1,"spt":2,"act":3}[args.branch]
    ref=model.prior.reference(
        max_tries=10000,
        warn_if_no_ref=True,
        random_state=np.random.RandomState(reference_seed),
    )
    ref=np.asarray(ref,dtype=float)
    if ref.ndim != 1 or ref.shape[0] != model.prior.d() or not np.all(np.isfinite(ref)):
        raise RuntimeError(
            f"REFERENCE_VECTOR_GATE=FAIL shape={ref.shape} expected=({model.prior.d()},)"
        )
    lp=model.logposterior(ref)
    if not math.isfinite(float(lp.logpost)):
        raise RuntimeError("FINITE_REFERENCE_GATE=FAIL")
    reference_preflight={
        "reference_is_pointlike":reference_is_pointlike,
        "pointlike_required":True,
        "seed":reference_seed,
        "sampled_parameter_order":list(model.prior.params),
        "sampled_reference_vector":[float(x) for x in ref],
        "finite_logposterior":True,
        "logposterior":float(lp.logpost),
    }
    # Actual minimisation.
    ...
    status.update({"status":"COMPLETE","chi2":chi,"minimum":row,
                   ...
                   "runtime_provenance_file":f"q016_v6_source_runtime/setup_{args.branch}.json"})
    write_json(out,status)

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--config",default=str(ROOT/"q016_matched_cmb_surface_v6_config.yml"))
