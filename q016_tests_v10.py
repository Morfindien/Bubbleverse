#!/usr/bin/env python3
"""Q016 V10 static/runtime objective tests and final gate validation."""
import argparse,ast,json,math
from pathlib import Path
import yaml

RUN="Q016-MATCHED-CMB-SURFACE-V10"
RESULT="R-Q016-EDE-MATCHED-CMB-SURFACE-010"
...
def static_test(cfg_path,output):
    ...
    src=Path("q016_objective_reprofile_v10.py").read_text()
    ast.parse(src)
    checks={
        ...
        "EXPLICIT_RESTART_OWNERSHIP_GATE":int(c["execution"]["optimizer"]["best_of"])==1,
        "MULTISTART_THRESHOLD_UNCHANGED_GATE":float(c["execution"]["continuation"]["multistart_delta_objective_max"])==1.0,
        "RESTART_COUNT_NOT_RELAXED_GATE":int(c["execution"]["continuation"]["minimum_distinct_complete_restarts"])==4,
        "Q011_NONPORTABILITY_GATE":c["model"]["q011_exact_vector_as_external_endpoint"] is False,
        "NO_LOWZ_GATE":c["surface"]["primary"]["low_z_data"]==[],
        "NO_LENSING_GATE":c["surface"]["primary"]["lensing"] is False,
        "NO_HARDCODED_ENDPOINT_PASS_GATE":'"ENDPOINT_REPROFILE_GATE":True' not in Path("q016_aggregate_v10.py").read_text().replace(" ",""),
        "PROFILE_NESTING_IMPLEMENTED_GATE":"profile_nesting_pass" in Path("q016_aggregate_v10.py").read_text(),
        "SEED_PRESERVATION_IMPLEMENTED_GATE":"EXACT_DETERMINISTIC_SEED" in src,
        "SHAPE_LIKELIHOOD_GATE":"q016_shape_tau_reio" in src and "make_shape_like" in src,
        "RECURSIVE_ARTIFACT_DISCOVERY_GATE":
            ".rglob(" in Path("q016_aggregate_v10.py").read_text(),
        "V9_PHASE1_PARENT_IDENTITY_GATE":
            'PARENT_FIXED_RUN="Q016-MATCHED-CMB-SURFACE-V9"' in Path("q016_aggregate_v10.py").read_text(),
        "NO_PHASE1_RERUN_POLICY_GATE":
            c["execution"]["continuation"]["phase1_execution_policy"]=="REUSE_AUTHENTICATED_V9_PHASE1_ARTIFACTS_DO_NOT_RERUN",
        "REMAINING_JOB_COUNT_GATE":
            int(c["execution"]["continuation"]["remaining_expensive_jobs"])==12,
    }
...
def runtime_objective_test(output):
    # Runtime semantics test for the exact V10 objective.
    #
    ...
    # the target lay ~7 sigma outside Cobaya's allowed numerical box. V10 instead
...
def main():
    ...
    p=sub.add_parser("static");p.add_argument("--config",default="q016_matched_cmb_surface_v10_config.yml");p.add_argument("--output",required=True)
