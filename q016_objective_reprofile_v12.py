#!/usr/bin/env python3
"""Bubbleverse Q016 V12 exact-objective fixed/free reprofile.

V12 changes only the computational objective implementation and continuation logic.
...
def run_task(args):
    ...
    status.update({"status":"RUNNING","seed_candidate":seed,"source_vector_meta":source_meta,
                   "runtime_provenance_file":f"q016_v12_source_runtime/setup_{branch}.json"})
    write_json(out,status)
...
def main():
    ...
    ap.add_argument("--config",default=str(ROOT/"q016_matched_cmb_surface_v12_config.yml"))
