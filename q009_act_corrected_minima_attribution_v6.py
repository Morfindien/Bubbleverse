...
DEFAULT_CONFIG = "q009_act_corrected_minima_attribution_v6_config.yml"
CODE_VERSION = "6.0-q006-preserved-baseline-vector"
...
def q006_preserved_q005_baseline(c: dict[str, Any], q006: Any) -> dict[str, Any]:
    if q006.get("q") != "Q-006":
        raise RuntimeError("Q006_BASELINE_VECTOR_GATE=FAIL wrong_q")

    models = q006.get("models", {})
    ...
    if not isinstance(profiles, list):
        raise RuntimeError("Q006_BASELINE_VECTOR_GATE=FAIL missing_profiles_list")
    ...
    for rec in profiles:
        if not isinstance(rec, dict):
            continue
        h0 = rec.get("target_h0")
        chi2 = rec.get("chi2_total", rec.get("chi2_scientific_total"))
        bestfit = rec.get("bestfit")
        if not isinstance(h0, (int, float)) or not isinstance(chi2, (int, float)):
            continue
        if not isinstance(bestfit, dict):
            continue
        if (
            math.isclose(float(h0), expected_h0, rel_tol=0.0, abs_tol=h0_tol)
            and math.isclose(float(chi2), expected_chi2, rel_tol=0.0, abs_tol=chi2_tol)
        ):
            matches.append(rec)

    if len(matches) != 1:
        raise RuntimeError(
            f"Q006_BASELINE_VECTOR_GATE=FAIL matches={len(matches)} "
            f"expected_H0={expected_h0} expected_chi2={expected_chi2}"
        )
    return matches[0]
...
def preflight(c: dict[str, Any], args) -> dict[str, Any]:
    q6 = load_json(args.q006) if Path(args.q006).is_file() else {}
    ...
    checks = {
        ...
        "PARENT_Q006_VECTOR_GATE": q6.get("q") == "Q-006",
    ...
    try:
        q006_preserved_q005_baseline(c, q6)
        checks["Q006_BASELINE_VECTOR_GATE"] = True
    except Exception:
        checks["Q006_BASELINE_VECTOR_GATE"] = False
...
def run_point(c: dict[str, Any], args) -> dict[str, Any]:
    ...
    baseline_rec = q006_preserved_q005_baseline(c, q6)
    ...
    result = {
        ...
        "reoptimized": False,
        "q005_baseline_reconstructed_from_q006_preserved_vector": {
            "H0": float(c["historical_baseline"]["h0"]),
            "chi2_scientific_total": float(baseline_rec.get("chi2_total", baseline_rec.get("chi2_scientific_total"))),
        ...
        "gates": {
            ...
            "Q006_PRESERVED_Q005_BASELINE_VECTOR_GATE": "PASS",
...
def main() -> int:
    ...
    p.add_argument("--q006", required=True)
